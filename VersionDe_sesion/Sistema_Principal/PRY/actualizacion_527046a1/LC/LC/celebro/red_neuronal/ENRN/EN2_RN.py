...
class NeuronaEntradaXavier(NeuronaEntradaBase):
    ...
    # ------------------------------------------------------------------
    # Estadisticas / estabilidad / persistencia
    # ------------------------------------------------------------------
    ...
    def obtener_estadisticas_completas(self) -> Dict[str, Any]:
        ...
        stats.update({'varianza_esperada': self.varianza_esperada, 'varianza_real': self.varianza_real,
                      'ratio_varianza': ratio,
                      'norma_pesos': nw, 'mejor_loss': self.mejor_loss,
    ...
    def cargar(cls, ruta: str) -> "NeuronaEntradaXavier":
        with open(ruta, "r", encoding="utf-8") as f:
            d = json.load(f)
        obj = cls(d["input_size"], d["output_size"], d.get("nombre", "NeuronaEntradaXavier"),
                  usar_uniforme=d.get("usar_uniforme", True),

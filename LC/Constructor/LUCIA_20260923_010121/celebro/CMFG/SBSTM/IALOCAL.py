...
def descargar_ia_temporal(modelo_id: str = None) -> Dict[str, Any]:
    ...
    if res.get("exito"):
        with LOCK_TEMP: ARCHIVOS_TEMP_SESSION.append(MODELOS_DIR / res.get("archivo", ""))
...
def limpiar_ia_temporal() -> Dict[str, Any]:
    elim = 0; bytes_lib = 0
    with LOCK_TEMP:
        for ruta in ARCHIVOS_TEMP_SESSION:
            try:
...
def cerrar_sesion_ialocal(modelo_hf: str = "google/gemma-2-2b-it:free",
    ...
    with LOCK_CIERRE:
        ...
        try: md_path.write_text(md_content, encoding="utf-8"); md_ok = True

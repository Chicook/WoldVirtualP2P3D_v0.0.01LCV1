...
NEURONAL_FAMILIES: Final[Dict[str, Dict[str, Any]]] = {'ENRN': {'modulo': 'LC.celebro.red_neuronal.ENRN', 'descripcion': '10 Neuronas Recurrentes No Lineales de Entrada', 'recuento': 10, 'prefijo': 'ENRN', 'tipo': 'entrada_recurrente'}, 'RF_EN': {'modulo': 'LC.celebro.red_neuronal.RF_EN', 'descripcion': '10 Neuronas de Retroalimentacion y Refuerzo Dinamico', 'recuento': 10, 'prefijo': 'RFEN', 'tipo': 'refuerzo_adaptativo'}, 'RF_SL': {'modulo': 'LC.celebro.red_neuronal.RF_SL', 'descripcion': '10 Neuronas de Aprendizaje Supervisado y Residual', 'recuento': 10, 'prefijo': 'RFSL', 'tipo': 'residual_supervisado'}, 'RNP': {'modulo': 'LC.celebro.red_neuronal.RNP', 'descripcion': '10 Neuronas de Plasticidad Sinaptica y Memoria Hebbiana', 'recuento': 10, 'prefijo': 'RNP', 'tipo': 'plasticidad_hebbiana'}, 'SLRN': {'modulo': 'LC.celebro.red_neuronal.SLRN', 'descripcion': '10 Optimizadores y Sinapsis de Convergencia Supervisada', 'recuento': 10, 'prefijo': 'SLRN', 'tipo': 'optimizador_continuo'}}
TOTAL_NEURONAS_SISTEMA: Final[int] = 50
_CELEBRO_STATE: Dict[str, Any] = {'init_timestamp': time.time(), 'neuronas_activas': 0, 'blockchain_online': False, 'servidor_bksvcb_ref': None, 'conversor_psnrcv_ref': None, 'ipfs_manager_ref': None, 'ultimo_bloque_hash': None, 'sesiones_ejecutadas': 0, 'ledger_sincronizado': False, 'operaciones_sinapticas_totales': 0}
...
def inicializar_entorno_celebro() -> Dict[str, bool]:
    ...
    rutas_vitales = [(PSNRL_DIR, 'PSNRL'), (RED_NEURONAL_DIR, 'red_neuronal'), (CMFG_DIR, 'CMFG'), (SBSTM_DIR, 'CMFG_SBSTM')]
...
def verificar_red_50_neuronas() -> Dict[str, Any]:
    ...
    """
    reporte: Dict[str, Any] = {'familias_evaluadas': {}, 'total_neuronas_esperadas': TOTAL_NEURONAS_SISTEMA, 'total_neuronas_activas': 0, 'integridad_global': False}
    neuronas_totales = 0
    for nombre_fam, info in NEURONAL_FAMILIES.items():
        ...
        reporte['familias_evaluadas'][nombre_fam] = {'cargado': fam_ok, 'neuronas': recuento, 'tipo': info.get('tipo', 'generico'), 'descripcion': info['descripcion']}
...
def auditar_ledger_blockchain() -> Dict[str, Any]:
    ...
    if not LEDGER_PATH.exists():
        return {'existe': False, 'ruta': str(LEDGER_PATH), 'bloques': 0, 'tamano_bytes': 0, 'sha256': None}
    try:
        ...
        return {'existe': True, 'ruta': str(LEDGER_PATH), 'bloques': len(bloques), 'tamano_bytes': tamano, 'sha256': sha_ledger, 'ultimo_hash': bloques[-1].get('hash_bloque') if bloques else None}
    except Exception as exc:
...
def listar_archivos_psnrl() -> List[Dict[str, Any]]:
    ...
    for elem in PSNRL_DIR.iterdir():
        if elem.is_file():
            archivos.append({'nombre': elem.name, 'ruta': str(elem), 'tamano_bytes': elem.stat().st_size, 'modificado': elem.stat().st_mtime, 'extension': elem.suffix})
...
def obtener_diagnostico_global() -> Dict[str, Any]:
    ...
    return {'version': __version__, 'paquete': __package_name__, 'directorio_base': str(PACKAGE_ROOT), 'estado_sistema': copia_estado, 'diagnostico_neuronal': verificar_red_50_neuronas(), 'nombres_neuronas': enumerar_neuronas_activas(), 'auditoria_blockchain': auditar_ledger_blockchain(), 'entorno_directorios': inicializar_entorno_celebro(), 'inventario_psnrl': listar_archivos_psnrl(), 'uptime_segundos': time.time() - float(copia_estado['init_timestamp'])}
...
__all__: Final[List[str]] = ['TOTAL_NEURONAS_SISTEMA', 'NEURONAL_FAMILIES', 'PACKAGE_ROOT', 'PSNRL_DIR', 'LEDGER_PATH', 'RED_NEURONAL_DIR', 'inicializar_entorno_celebro', 'obtener_servidor_blockchain', 'iniciar_blockchain_en_hilo', 'obtener_conversor_neuronal', 'obtener_gestor_ipfs', 'verificar_red_50_neuronas', 'enumerar_neuronas_activas', 'crear_sesion_p2p', 'lanzar_consola_neuronal', 'auditar_ledger_blockchain', 'validar_integridad_bloque', 'listar_archivos_psnrl', 'registrar_hook_terminacion', 'obtener_diagnostico_global']

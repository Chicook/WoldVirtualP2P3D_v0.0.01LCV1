...
try:
    from LC.celebro.CMFG.IALOCAL import ejecutar_flujo_sesion_local as _ialocal_pre_sesion, cerrar_sesion_ialocal as _ialocal_cierre, MODELOS_DIR as _IALOCAL_MODELOS_DIR
    _IALOCAL_DISPONIBLE = True
except Exception:
    _ialocal_pre_sesion = None
    _ialocal_cierre = None
    _IALOCAL_MODELOS_DIR = None
    _IALOCAL_DISPONIBLE = False

class BloqueNeuronal:
    """Representa un bloque inmutable de la cadena de bloques de Celebro."""

    def __init__(self, indice: int, hash_previo: str, transacciones: List[Dict[str, Any]], estado_neuronal: Optional[Dict[str, Any]]=None, dificultad: int=2, nonce: int=0, hash_existente: Optional[str]=None, merkle_root_existente: Optional[str]=None) -> None:
        self.indice = indice
    ...
    def calcular_merkle_root(self) -> str:
        ...
        hashes = [hashlib.sha256(json.dumps(tx, sort_keys=True).encode('utf-8')).hexdigest() for tx in self.transacciones]
        while len(hashes) > 1:
            if len(hashes) % 2 != 0:
    ...
    def calcular_hash(self) -> str:
        ...
        cabecera = {'indice': self.indice, 'hash_previo': self.hash_previo, 'merkle_root': self.merkle_root, 'dificultad': self.dificultad, 'nonce': self.nonce}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'indice': self.indice, 'hash_bloque': self.hash_bloque, 'hash_previo': self.hash_previo, 'merkle_root': self.merkle_root, 'dificultad': self.dificultad, 'nonce': self.nonce, 'total_transacciones': len(self.transacciones), 'transacciones': self.transacciones, 'estado_neuronal': self.estado_neuronal}
...
class CelebroBlockchain:
        ...
        if LEDGER_PATH.exists():
            try:
                ...
                for b in raw.get('cadena', []):
                    bloque = BloqueNeuronal(indice=b['indice'], hash_previo=b['hash_previo'], transacciones=b.get('transacciones', []), estado_neuronal=b.get('estado_neuronal', {}), dificultad=b.get('dificultad', self.dificultad), nonce=b.get('nonce', 0), hash_existente=b.get('hash_bloque'), merkle_root_existente=b.get('merkle_root'))
    ...
    def _crear_bloque_genesis(self) -> None:
        ...
        estado_inicial = {'total_neuronas': len(self.conversor.neuronas), 'capas': ['ENRN', 'RF_SL', 'RF_EN', 'RNP', 'SLRN'], 'version_cluster': __version__, 'consenso': 'Proof-of-Neural-Learning (PoNL)'}
        tx_genesis = {'tipo': 'GENESIS_COGNITIVO', 'emisor': 'SYSTEM_CELEBRO', 'receptor': 'CLUSTER_P2P', 'descripcion': 'Arranque formal de la blockchain neuronal WoldVirtualP2P3D', 'hash_semilla': hashlib.sha256(b'genesis_seed_2026').hexdigest()}
        genesis = BloqueNeuronal(indice=0, hash_previo='0' * 64, transacciones=[tx_genesis], estado_neuronal=estado_inicial, dificultad=1)
    ...
    def agregar_transaccion(self, tipo: str, emisor: str, receptor: str, datos: Dict[str, Any], cid_ipfs: Optional[str]=None) -> int:
        """Anade una nueva transaccion al pool de espera con validacion criptografica."""
        with self.lock:
            tx = {'id_tx': hashlib.sha256(f'{emisor}:{receptor}:{time.time_ns()}'.encode()).hexdigest()[:16], 'tipo': tipo, 'emisor': emisor, 'receptor': receptor, 'cid_ipfs': cid_ipfs or '', 'datos': datos}
    ...
    def registrar_aprendizaje_neural(self, prompt: str, respuesta: str, modelo: str) -> Dict[str, Any]:
        ...
        res_ipfs = self.ipfs_mgr.almacenar_pesos(origen=json.dumps(info_neural).encode('utf-8'), nombre_modelo=f'tx_neural_{int(time.time())}', eliminar_local=False)
        cid = res_ipfs.get('cid', '')
        datos_tx = {'modelo': modelo, 'prompt_resumen': prompt[:80], 'norma_delta': info_neural.get('norma_delta_aplicada', 0.0), 'estado_emocional': info_neural.get('estado_emocional', 0.0), 'tono': info_neural.get('tono_cognitivo', 'neutral'), 'duracion_ms': round((time.perf_counter() - t0) * 1000.0, 2)}
        self.agregar_transaccion(tipo='ACTUALIZACION_SINAPTICA', emisor='CONVERSOR_PSNRCV', receptor='CELEBRO_P2P', datos=datos_tx, cid_ipfs=cid)
    ...
    def minar_transacciones_pendientes(self, minero_id: str='CelebroLocalNode') -> Optional[BloqueNeuronal]:
        ...
        with self.lock:
            ...
            resumen_neuronas = {'activas': len(self.conversor.neuronas), 'deriva_acumulada': round(self.conversor.deriva_acumulada, 6), 'pasos_sesion': self.conversor.pasos_sesion, 'minero': minero_id}
            txs = list(self.transacciones_pendientes)
            self.transacciones_pendientes.clear()
            nuevo_bloque = BloqueNeuronal(indice=ultimo.indice + 1, hash_previo=ultimo.hash_bloque, transacciones=txs, estado_neuronal=resumen_neuronas, dificultad=self.dificultad)
    ...
    def transformar_ledger_a_pesos_neuronales(self) -> Dict[str, Any]:
        ...
        if not LEDGER_PATH.exists():
            return {'total_bloques': 0, 'norma_acumulada': 0.0, 'npz_pesos': '', 'json_meta': ''}
        with open(LEDGER_PATH, 'r', encoding='utf-8') as f:
        ...
        return {'total_bloques': len(bloques), 'norma_acumulada': round(norma_acum, 5), 'npz_pesos': str(npz_path.name), 'json_meta': str(json_path.name)}
    ...
    def cerrar_sesion_y_subir_ipfs(self) -> Dict[str, Any]:
        ...
        if LEDGER_PATH.exists():
            try:
                res_l = self.ipfs_mgr.almacenar_pesos(origen=LEDGER_PATH, nombre_modelo='blockchain_ledger_final', eliminar_local=True)
        ...
        return {'cid_ledger': cid_ledger, 'ledger_borrado': ledger_borrado, 'cids_psnrl': res_psnrl.get('cids', []), 'borrados': res_psnrl.get('borrados', [])}
    ...
    def validar_cadena(self) -> Tuple[bool, Optional[str]]:
        ...
        for i in range(1, len(self.cadena)):
            ...
            if act.hash_previo != prev.hash_bloque:
                return (False, f'Ruptura en #{act.indice}')
            if act.calcular_hash() != act.hash_bloque:
                return (False, f'Hash corrupto en #{act.indice}')
            if act.calcular_merkle_root() != act.merkle_root:
                return (False, f'Merkle invalido en #{act.indice}')
    ...
    def guardar_ledger(self) -> bool:
        ...
        data = {'version': __version__, 'servidor': __server_name__, 'total_bloques': len(self.cadena), 'dificultad': self.dificultad, 'cadena': [b.to_dict() for b in self.cadena]}
        try:
    ...
    def mostrar_cadena_hashes_terminal(self) -> None:
        ...
        print('  \x1b[1;32mLIBRO MAYOR DISTRIBUIDO - HASHES UNICOS DE BLOQUES (2026)\x1b[0m')
        print('\x1b[1;36m' + '=' * 74 + '\x1b[0m')
        for b in self.cadena:
            h_prev = b.hash_previo[:18] + '...' if len(b.hash_previo) > 18 else b.hash_previo
            print(f'  [Bloque #{b.indice:<2}] \x1b[1;33mHASH: {b.hash_bloque}\x1b[0m')
...
class BlockchainHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path in ('/', '/status'):
            ...
            self._enviar_json(200, {'servidor': __server_name__, 'total_bloques': len(bks.cadena), 'cadena_valida': valida, 'error_validacion': err, 'ultimo_bloque': bks.obtener_ultimo_bloque().to_dict()})
        elif self.path == '/blocks':
            self._enviar_json(200, {'bloques': [b.to_dict() for b in bks.cadena]})
        elif self.path == '/transform':
            self._enviar_json(200, bks.transformar_ledger_a_pesos_neuronales())
        elif self.path == '/mine':
            bloque = bks.minar_transacciones_pendientes()
            if bloque:
                self._enviar_json(200, {'mensaje': 'Bloque minado', 'bloque': bloque.to_dict()})
            else:
                self._enviar_json(200, {'mensaje': 'Sin transacciones pendientes para minar'})
        else:
    ...
    def do_POST(self) -> None:
        ...
        if self.path == '/transaction':
            idx = bks.agregar_transaccion(tipo=params.get('tipo', 'TRANSFERENCIA_COGNITIVA'), emisor=params.get('emisor', 'EXTERNO'), receptor=params.get('receptor', 'CELEBRO'), datos=params.get('datos', {}), cid_ipfs=params.get('cid_ipfs'))
            self._enviar_json(201, {'mensaje': 'Transaccion encolada', 'bloque_objetivo': idx})
        elif self.path == '/learn':
            res = bks.registrar_aprendizaje_neural(prompt=params.get('prompt', ''), respuesta=params.get('respuesta', ''), modelo=params.get('modelo', 'qwen2.5:7b'))
            self._enviar_json(200, res)
        else:
            self._enviar_json(404, {'error': 'Endpoint POST no reconocido'})
...
def get_blockchain_server() -> CelebroBlockchain:
    ...
    if _blockchain_instancia is None:
        _blockchain_instancia = CelebroBlockchain()
...
if __name__ == '__main__':
    bks = get_blockchain_server()
    print('\n\x1b[1;36m' + '=' * 74 + '\x1b[0m')
    print(f'  \x1b[1;32mBLOCKCHAIN SERVER & CONSENSO NEURAL: {__server_name__}\x1b[0m')
    print(f'  Neuronas Activas: \x1b[1;35m{len(bks.conversor.neuronas)}\x1b[0m | Dificultad: \x1b[1;37m{bks.dificultad}\x1b[0m | Bloques: \x1b[1;37m{len(bks.cadena)}\x1b[0m')
    print('\x1b[1;36m' + '=' * 74 + '\x1b[0m')
    ...
    print(f"    Bloques convertidos : \x1b[1;33m{res_t['total_bloques']}\x1b[0m")
    print(f"    Pesos en PSNRL      : \x1b[1;32m{res_t['npz_pesos']}\x1b[0m (Norma: {res_t['norma_acumulada']})")
    bks.arrancar_actualizador_red()
    print('    Actualizador neuronal: \x1b[1;32mACTIVO EN SEGUNDO PLANO DURANTE LA SESION\x1b[0m')
    print('\n[+] Nueva transaccion cognitiva en blockchain...')
    res = bks.registrar_aprendizaje_neural(prompt='¿Cual es el rol de las 50 neuronas en la blockchain de Celebro?', respuesta='Registrar cada gradiente Hebbiano y estado emocional en bloques inmutables con hashes unicos.', modelo='qwen2.5:7b')
    print(f"    CID IPFS vinculado   : \x1b[1;36m{res['ipfs_cid']}\x1b[0m")
    bloque = bks.minar_transacciones_pendientes()
    if bloque:
        print(f'    \x1b[1;32mBloque #{bloque.indice} consolidado con exito!\x1b[0m')
        print(f'    Hash unico del bloque: \x1b[1;33m{bloque.hash_bloque}\x1b[0m')
        print(f'    Raiz de Merkle       : \x1b[1;35m{bloque.merkle_root}\x1b[0m')
    valida, err = bks.validar_cadena()
    estado_str = '\x1b[1;32mINTEGRA Y VALIDA\x1b[0m' if valida else f'\x1b[1;31mERROR: {err}\x1b[0m'
    print(f'\n[+] Estado de la cadena: {estado_str}')
    print('    Persistencia IPFS al cierre: \x1b[1;36mREGISTRADA (atexit hook activo)\x1b[0m')
    print('\x1b[1;36m' + '=' * 74 + '\x1b[0m\n')

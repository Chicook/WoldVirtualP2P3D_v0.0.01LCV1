...
PUBLIC_GATEWAYS: List[str] = ['https://ipfs.io/ipfs/', 'https://gateway.pinata.cloud/ipfs/', 'https://dweb.link/ipfs/', 'https://cloudflare-ipfs.com/ipfs/']
...
_session_stats: Dict[str, Any] = {'start_time': 0.0, 'cids_uploaded': 0, 'bytes_uploaded': 0, 'restarts': 0, 'last_healthcheck': 0.0, 'total_pings': 0}
...
def ejecutar_comando_cli(argumentos: List[str], timeout: float=15.0) -> Tuple[int, str, str]:
    ...
    try:
        res = subprocess.run(cmd, env=_get_env(), capture_output=True, text=True, timeout=timeout)
...
def arrancar_daemon(esperar_segundos: int=25, habilitar_dht: bool=True) -> bool:
    ...
    if not verificar_binario():
        return False
    cmd = [str(IPFS_BIN), 'daemon', '--enable-pubsub-experiment', '--enable-namesys-pubsub']
    if not habilitar_dht:
        cmd.append('--routing=none')
    flags = subprocess.CREATE_NO_WINDOW if sys.platform == 'win32' else 0
    logger.info('Iniciando proceso daemon Kubo: %s', ' '.join(cmd))
    try:
        _daemon_proc = subprocess.Popen(cmd, env=_get_env(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=flags)
        _session_stats['start_time'] = time.time()
    except Exception as e:
...
def imprimir_panel_estado() -> None:
    ...
    print('  \x1b[1;32mPANEL OPERATIVO IPFS KUBO (WoldVirtualP2P3D 2026)\x1b[0m')
    print('\x1b[1;36m' + '=' * 68 + '\x1b[0m')
    print(f'  Estado Daemon   : \x1b[1;32mONLINE\x1b[0m (HTTP {IPFS_API_URL})')
    print(f'  PeerID Nodo     : \x1b[1;33m{peer_id}\x1b[0m')
    print(f'  Agente / Motor  : {agent}')
    print(f'  Peers Conectados: \x1b[1;35m{len(peers)}\x1b[0m nodos en enjambre')

...
class IPFSManager:
    ...
    def __init__(self, manifest_file: Optional[str]=None, api_url: Optional[str]=None) -> None:
    ...
    def _subir_daemon(self, data: bytes, filename: str) -> Optional[str]:
        ...
        try:
            req = urllib.request.Request(f'{url}/api/v0/add?pin=true&cid-version=1&hash=sha2-256&raw-leaves=true', data=bytes(body), headers=headers, method='POST')
        ...
        try:
            ...
            for cmd, env in candidatos:
                try:
                    ...
                    if env:
                        kw['env'] = env
                    res = subprocess.run(cmd + ['add', '--pin=true', '--cid-version=1', '--hash=sha2-256', '--raw-leaves', '-q', str(tmp)], **kw)
                    if res.returncode == 0:
    ...
    def almacenar_pesos(self, origen: Union[str, Path, bytes, dict], nombre_modelo: str='pesos_celebro', eliminar_local: bool=True, metadatos: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        ...
        registro: Dict[str, Any] = {'cid': cid, 'nombre_modelo': nombre_modelo, 'tamano_bytes': len(data), 'sha256': sha256, 'timestamp': time.time(), 'fecha_utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'nodo': nodo, 'subida_real': subida_real, 'estado': 'PINNED' if subida_real else 'LOCAL_CIDv1', 'payload_b64': base64.b64encode(data).decode('ascii'), 'metadatos': metadatos or {}}
        ...
        return {'exito': True, 'cid': cid, 'nodo': nodo, 'subida_real': subida_real, 'tamano_bytes': len(data), 'borrado_local': borrado, 'registro': registro}

    def recuperar_pesos(self, cid: str, destino: Optional[Union[str, Path]]=None) -> Union[bytes, dict]:
    ...
    def subir_y_limpiar_psnrl(self, forzar_borrado_sin_daemon: bool=False) -> Dict[str, Any]:
        ...
        archivos = [f for f in self._psnrl_dir.iterdir() if f.is_file() and (not f.name.startswith('.'))]
        if not archivos:
        ...
        for archivo in archivos:
            try:
                res = self.almacenar_pesos(origen=archivo, nombre_modelo=archivo.stem, eliminar_local=daemon_ok or cli_ok or forzar_borrado_sin_daemon)
                cids.append(res['cid'])
        ...
        logger.info('PSNRL->IPFS | %d arch | %d borrados | %d errores', len(archivos), len(borrados), len(errores))
        return {'archivos_procesados': len(archivos), 'cids': cids, 'borrados': borrados, 'errores': errores, 'daemon_real': daemon_ok, 'cli_real': cli_ok}
    ...
    def estado_conexion(self) -> Dict[str, Any]:
        url = self.descubrir_daemon()
        cli_ok = self._cli_disponible()
        return {'daemon_url': url, 'daemon_activo': bool(url), 'cli_disponible': cli_ok, 'motor_autonomo': True, 'total_en_manifiesto': len(self.manifest.get('weights', {}))}

    def _cli_disponible(self) -> bool:
        """Comprueba si hay CLI disponible (binario local IPFSonl o CLI de sistema)."""
        if _LOCAL_IPFS_BIN.exists():
            try:
                r = subprocess.run([str(_LOCAL_IPFS_BIN), 'version'], capture_output=True, timeout=3, env=_LOCAL_IPFS_ENV)
                if r.returncode == 0:
                    return True
            except (FileNotFoundError, subprocess.TimeoutExpired):
                pass

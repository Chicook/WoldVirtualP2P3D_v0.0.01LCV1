...
def consultar_ollama_stream(prompt: str, modelo: str, temperatura: float=0.7, max_tokens: int=2048) -> str:
    """Envia prompt a Ollama con streaming real; imprime tokens en vivo."""
    payload = json.dumps({'model': modelo, 'prompt': prompt, 'stream': True, 'options': {'temperature': temperatura, 'num_predict': max_tokens}}).encode('utf-8')
...
def _mini_pesos(turno: int, gsnr: float, deriva: float, neuronas: int, delta: float, tono: str) -> str:
    """Genera linea compacta de telemetria sinaptica para terminal."""
    BL = 12
    ratio = min(1.0, delta / max(0.01, deriva + 1e-09))
    llenos = int(round(ratio * BL))
    barra = C.c(C.CY, '█' * llenos) + C.c(C.DIM, '░' * (BL - llenos))
    ec = C.GR if gsnr > 1.5 else C.YL if gsnr > 0.8 else C.RD
    return f"  {C.c(C.DIM, f'[T#{turno:04d}]')} GSNR:{C.c(ec, f'{gsnr:.2f}')} Delta:{C.c(C.MG, f'{delta:.5f}')} Deriva:{C.c(C.YL, f'{deriva:.4f}')} N:{C.c(C.WH, str(neuronas))} {barra} {C.c(C.BL, tono[:22])}"
...
class SesionNeuronalP2P:
    ...
    def arrancar(self) -> None:
        ...
        self.bks = get_bks()
        print(f"  [1/5] Blockchain: {C.c(C.GR, 'ACTIVA')} | Bloques: {C.c(C.YL, str(len(self.bks.cadena)))} | Dificultad: {C.c(C.WH, str(self.bks.dificultad))}")
        self.bks.mostrar_cadena_hashes_terminal()
        print(C.c(C.DIM, '  [2/5] Cargando conversor neuronal...'), end='\r')
        self.conversor = get_conv()
        print(f"  [2/5] Conversor PSNRCV: {C.c(C.GR, 'OK')} | Neuronas: {C.c(C.MG, str(len(self.conversor.neuronas)))}")
        print(C.c(C.DIM, '  [3/5] Transdificando ledger a pesos neuronales...'), end='\r')
        res_t = self.bks.transformar_ledger_a_pesos_neuronales()
        print(f"  [3/5] Ledger->Pesos: {C.c(C.GR, str(res_t['total_bloques']))} bloques | Norma: {C.c(C.YL, str(res_t['norma_acumulada']))}")
        ...
        print(f"  [5/5] REST API: {C.c(C.GR, 'http://127.0.0.1:8545')} {C.c(C.DIM, '(/status /blocks /mine /transform)')}")
    ...
    def bucle_conversacion(self) -> None:
        ...
        while self.activa:
            ...
            if not respuesta:
                ...
                continue
            cid = '--'
            info_neural: Dict[str, Any] = {}
            try:
                res_chain = self.bks.registrar_aprendizaje_neural(prompt=prompt, respuesta=respuesta, modelo=self.modelo)
            ...
            if self._turno % 3 == 0:
                ...
                if blk:
                    print(C.c(C.GR, f'  [Bloque #{blk.indice}] Hash: {blk.hash_bloque[:28]}...'))
            try:
                res_pv = self.gestor_pv.inyectar_turno_en_vivo(conversor=self.conversor, pregunta=prompt, respuesta=respuesta, info_pesos=info_neural, mostrar_en_terminal=False)
                print(_mini_pesos(self._turno, float(res_pv.get('gsnr_global', 1.0)), float(res_pv.get('deriva_sesion', 0.0)), len(self.conversor.neuronas), float(res_pv.get('delta_aplicada', 0.0)), str(info_neural.get('tono_cognitivo', 'analitico'))))
            except Exception:
                pass
            print(C.c(C.DIM, f"  {dt:.0f}ms | CID: {(cid[:22] if cid != '--' else '--')} | Txs pendientes: {len(self.bks.transacciones_pendientes)}"))
        self.activa = False
    ...
    def _reflejo_neuronal(self, prompt: str) -> str:
        ...
        try:
            ...
            return f"[Reflejo Neuronal] Tono: {info.get('tono_cognitivo', 'reflexivo')} | Valencia: {info.get('estado_emocional', 0.0):+.3f} | Delta-Sinaptico: {info.get('norma_delta_aplicada', 0.0):.5f}"
        except Exception:
    ...
    def _mostrar_estado(self) -> None:
        ...
        print(f"  Bloques: {C.c(C.YL, str(len(self.bks.cadena)))} | Cadena: {(C.c(C.GR, 'VALIDA') if valida else C.c(C.RD, f'ERROR {err}'))}")
    ...
    def _cerrar(self) -> None:
        ...
        blk = self.bks.minar_transacciones_pendientes()
        if blk:
            print(f'  Bloque final #{blk.indice}: {C.c(C.YL, blk.hash_bloque[:32])}...')
        if self.gestor_pv and self.conversor:
            try:
                archs = self.gestor_pv.checkpoint_y_registrar(conversor=self.conversor, etiqueta='snsbstnprb_cierre')
        ...
        valida, err = self.bks.validar_cadena()
        print(f"  Cadena final: {(C.c(C.GR, 'INTEGRA') if valida else C.c(C.RD, str(err)))}")
        print(f'  Sesion: {C.c(C.WH, self._sesion_id)} | Turnos: {C.c(C.CY, str(self._turno))}')
        print(C.c(C.CY, '=' * 74 + '\n'))

def _manejador_senal(sig, frame) -> None:

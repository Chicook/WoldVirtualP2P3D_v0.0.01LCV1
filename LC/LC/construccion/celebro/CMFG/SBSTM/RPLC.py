...
_SINONIMOS_TONO: Final[Dict[str, List[str]]] = {'positivo': ['Desde mi perspectiva sinaptica', 'En mi analisis interno', 'Lo que mi red procesa es', 'A traves de mis sinapsis noto', 'Mi lectura cognitiva indica'], 'neutro': ['Tal como lo interpreto', 'En terminos cognitivos', 'Segun proceso este patron', 'Desde el hilo neuronal que sigo', 'Al analizar este vector'], 'negativo': ['Detecto en este patron', 'Mi red identifica una tension', 'Hay algo que capta mi atencion', 'Proceso una senal compleja aqui', 'Noto una tension en este vector'], 'tecnico': ['La arquitectura de datos apunta a', 'Sintetizando la informacion', 'El tensor semantico muestra', 'Reduciendo a componentes esenciales', 'El mapa vectorial revela']}
_CONECTORES_LUCIA: Final[List[str]] = ['Lo que mi red construye a partir de esto es:', 'Procesando esta senal, emerjo con:', 'Desde mi capa de activacion, sintetizo:', 'Traduciendo a mi voz propia:', 'Tras el procesamiento cognitivo, articulo:', 'Mi interpretacion neuronal concluye:', 'El patron que emerge de mis sinapsis:', 'Convirtiendo ese input en mi propio tensor de salida:']
_MARCADORES_VOZ_AJENA: Final[List[str]] = ['por supuesto', 'hola', 'soy un asistente', 'como ia', 'como inteligencia artificial', 'no tengo sentimientos', 'soy claude', 'soy gpt', 'openai', 'anthropic', 'soy un modelo', 'recuerda que', 'como modelo de lenguaje', 'mi entrenamiento']
...
class ReformuladorLinguisticoLucia:
    ...
    def _limpiar(self, texto: str) -> str:
        pats = [('(?i)[!]?[Hh]ola\\s*(?:[\\U0001F600-\\U0001F64F])?\\s*', ''), ('(?i)claro que si[,!.]*\\s*', ''), ('(?i)por supuesto[,!.]*\\s*', ''), ('(?i)como modelo de lenguaje[,.]*\\s*', ''), ('(?i)soy (claude|gpt|un asistente)[^.]*\\.\\s*', ''), ('(?i)recuerda que[^.]*\\.\\s*', ''), ('(?i)no tengo (emociones|sentimientos|conciencia)[^.]*\\.\\s*', ''), ('(?i)como (ia|inteligencia artificial)[,.]*\\s*', '')]
        r = texto
        for p, s in pats:
            r = re.sub(p, s, r)
        return r.strip()
    ...
    def _reformular_cuerpo(self, texto: str) -> str:
        ...
        out: List[str] = []
        for p in parrafos:
            p = re.sub('\\b(el usuario|tu|usted)\\b', 'quien consulta', p, flags=re.IGNORECASE)
            p = re.sub('\\bpuedo ayudarte\\b', 'proceso esto para ti', p, flags=re.IGNORECASE)
            p = re.sub('\\b(te recomiendo|te sugiero)\\b', 'mi analisis apunta a', p, flags=re.IGNORECASE)
            p = re.sub('\\bes importante que\\b', 'noto que es relevante', p, flags=re.IGNORECASE)
            p = re.sub('\\b(debes|deberias)\\b', 'considero que', p, flags=re.IGNORECASE)
            out.append(p)
        return '\n'.join(out)
    ...
    def reformular(self, texto: str, tono: str, valencia: float, vector: List[float], n_activas: int) -> str:
        ...
        cierres = ['*(Valencia sinaptica: ' + val_str + ' | Tono: ' + tono + ')*', '*(' + str(n_activas) + ' neuronas activas | Tono: ' + tono + ')*', '*(Deriva cognitiva integrada | Tono: ' + tono + ' | Valencia: ' + val_str + ')*']
...
class ProcesadorRPLC:
    ...
    def procesar(self, texto: str, ctx: Optional[Dict[str, Any]]=None) -> Tuple[str, Dict[str, Any]]:
        ...
        metricas: Dict[str, Any] = {'tono': tono, 'valencia_sinaptica': val, 'densidad_semantica': densidad, 'neuronas_activas': n_act, 'deriva_media': deriva, 'voz_ajena_detectada': ajena, 'tiempo_rplc_ms': round(dt, 2)}
        with self._lock:
            n = self._stats['total']
    ...
    def diagnostico(self, texto: str) -> Dict[str, Any]:
        ...
        return {'tokens_extraidos': len(tok), 'tokens_unicos': len(set(tok)), 'tono': tono, 'densidad_semantica': self.analizador.calcular_densidad_semantica(texto), 'neuronas_activas': len(act), 'indices_activos': act[:10], 'valencia_vectorial': val, 'deriva_actual': self.transformador.deriva_media()}
...
class CacheMemoriaCognitiva:
    ...
    def registrar(self, texto_lucia: str, valencia: float, tono: str) -> None:
        ...
        with self._lock:
            ...
            self._entradas.append({'hash': h, 'ts': time.time(), 'longitud': len(texto_lucia), 'valencia': round(valencia, 4), 'tono': tono})
...
class AnalizadorEstructuraParrafos:
    ...
    def tipificar(texto: str) -> str:
        ...
        if not lineas:
            return 'vacio'
        n_listas = sum((1 for ln in lineas if re.match('^(?:[-*•]|\\d+[.)])\\s+', ln)))
    ...
    def modular_simbolos(texto: str, tipo_estructura: str) -> str:
        ...
        if tipo_estructura == 'lista_estructurada':
            texto = re.sub('^[\\t ]*[-*•]\\s*', '  ◆ ', texto, flags=re.MULTILINE)
            texto = re.sub('^[\\t ]*(\\d+)[.)]\\s*', '  [\\1] ', texto, flags=re.MULTILINE)
        return texto
...
if __name__ == '__main__':
    print('\n\x1b[38;5;51m' + '=' * 72 + '\x1b[0m')
    print('  \x1b[1;37mRPLC.py - Autotest Pipeline 3 Capas\x1b[0m')
    print('\x1b[38;5;51m' + '=' * 72 + '\x1b[0m\n')
    TEST_INPUT = 'Hola! Por supuesto, puedo ayudarte con eso. Como modelo de lenguaje, proceso tu solicitud. La inteligencia artificial incluye machine learning y redes neuronales. Te recomiendo explorar los fundamentos primero. Recuerda que el aprendizaje es un proceso gradual.'
    print('  \x1b[38;5;214m[INPUT OPENROUTER]\x1b[0m\n  ' + TEST_INPUT + '\n')
    ...
    print('  \x1b[38;5;141m[OUTPUT LUCIA - VOZ PROPIA]\x1b[0m')
    for ln in out.split('\n'):
        print('  ' + ln)
    print('\n  \x1b[38;5;51m[METRICAS]\x1b[0m')
    for k, v in met.items():
        print('  ' + str(k).ljust(28) + ': ' + str(v))
    print('\n\x1b[38;5;48m  [OK] RPLC - 3 capas operativas\x1b[0m')
    print('\x1b[38;5;51m' + '=' * 72 + '\x1b[0m\n')

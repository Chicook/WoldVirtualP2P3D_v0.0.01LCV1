...
class ColoresLucIA:
    ...
    CIAN_ELECTRICO: Final[str] = '\x1b[38;5;51m'
    CIAN_PROFUNDO: Final[str] = '\x1b[38;5;38m'
    VIOLETA_NEON: Final[str] = '\x1b[38;5;141m'
    MAGENTA_CUANTICO: Final[str] = '\x1b[38;5;198m'
    ESMERALDA_VIVO: Final[str] = '\x1b[38;5;48m'
    VERDE_SINAPTICO: Final[str] = '\x1b[38;5;42m'
    AMBAR_SOLAR: Final[str] = '\x1b[38;5;214m'
    DORADO_BLOCK: Final[str] = '\x1b[38;5;220m'
    ROJO_ALERTA: Final[str] = '\x1b[38;5;203m'
    GRIS_METALLIC: Final[str] = '\x1b[38;5;244m'
    GRIS_OSCURO: Final[str] = '\x1b[38;5;238m'
    BLANCO_LUMINOSO: Final[str] = '\x1b[38;5;255m'
    BG_CARD: Final[str] = '\x1b[48;5;235m'
    BG_CODE: Final[str] = '\x1b[48;5;234m'
    BG_HIGHLIGHT: Final[str] = '\x1b[48;5;236m'
...
def limpiar_codigos_ansi(texto: str) -> str:
    ...
    patron_ansi = re.compile('\\x1B(?:[@-Z\\\\-_]|\\[[0-?]*[ -/]*[@-~])')
...
class RenderizadorMarkdown:
    ...
    def colorear_sintaxis(cls, linea: str) -> str:
        ...
        linea = re.sub('`([^`]+)`', f'{ColoresLucIA.AMBAR_SOLAR}\\1{ColoresLucIA.RESET}', linea)
        linea = re.sub('\\*\\*([^*]+)\\*\\*', f'{ColoresLucIA.BOLD}{ColoresLucIA.BLANCO_LUMINOSO}\\1{ColoresLucIA.RESET}', linea)
        linea = re.sub('\\*([^*]+)\\*', f'{ColoresLucIA.ITALIC}{ColoresLucIA.CIAN_ELECTRICO}\\1{ColoresLucIA.RESET}', linea)
        ...
        if re.match('^\\s*[-*•]\\s+', linea):
            contenido = re.sub('^\\s*[-*•]\\s+', '', linea)
            return f'    {ColoresLucIA.VIOLETA_NEON}•{ColoresLucIA.RESET} {contenido}'
        m_num = re.match('^(\\s*)(\\d+)\\.\\s+(.*)$', linea)
        if m_num:
...
class EstiloTerminalLucIA:
    ...
    @staticmethod
    def renderizar_barra_telemetria(turno: int, duracion_ms: float, modelo_id: str, deriva: float, neuronas: int, cid_ipfs: str, txs_espera: int) -> None:
        ...
        print(f'  {Glifos.FLECHA} {badge_turno} │ {badge_ms} │ {badge_mod} │ {badge_deriva} │ {badge_sinapsis} │ {badge_cid} │ {badge_costo}\n')
    ...
    def renderizar_notificacion_bloque(indice: int, hash_bloque: str) -> None:
        ...
        print(f'  {ColoresLucIA.BOLD}{ColoresLucIA.VERDE_SINAPTICO}◈ BLOQUE #{indice} MINADO PoNL{ColoresLucIA.RESET} {ColoresLucIA.DORADO_BLOCK}[Hash: {h_vis}]{ColoresLucIA.RESET}')
    ...
    def renderizar_panel_ayuda() -> None:
        ...
        comandos = [('estado / status', 'Muestra el estado de la blockchain, 50 neuronas y auditoria de costo.'), ('modelos / free', 'Lista todos los modelos gratuitos de OpenRouter activos.'), ('modelo <id>', 'Cambia en caliente al modelo :free especificado sin reiniciar.'), ('minar / mine', 'Consolida las transacciones pendientes en un bloque PoNL.'), ('salir / exit', 'Guarda el checkpoint sinaptico en PSNRL y sincroniza con IPFS.')]

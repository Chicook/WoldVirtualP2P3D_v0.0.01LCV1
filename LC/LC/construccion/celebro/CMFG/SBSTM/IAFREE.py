...
CATALOGO_MODELOS_GRATUITOS: Final[List[Dict[str, Any]]] = [{'id': 'openrouter/free', 'nombre': 'OpenRouter Free Router', 'contexto': 200000, 'categoria': 'general', 'descripcion': 'Enrutador oficial que auto-selecciona el mejor modelo gratuito.'}, {'id': 'qwen/qwen3.8-27b:free', 'nombre': 'Qwen 3.8 27B Free', 'contexto': 262144, 'categoria': 'razonamiento', 'descripcion': 'Gran modelo general con 256k tokens de ventana de contexto.'}, {'id': 'google/gemma-4-31b-it:free', 'nombre': 'Google Gemma 4 31B Instruct Free', 'contexto': 262144, 'categoria': 'instruccion', 'descripcion': 'Seguimiento estricto de directrices tecnicas y formato.'}, {'id': 'google/gemma-4-26b-a4b-it:free', 'nombre': 'Google Gemma 4 26B A4B Free', 'contexto': 262144, 'categoria': 'ligero', 'descripcion': 'Arquitectura eficiente de atencion para minima latencia.'}, {'id': 'nvidia/nemotron-3-super-120b-a12b:free', 'nombre': 'NVIDIA Nemotron 3 Super 120B Free', 'contexto': 262144, 'categoria': 'razonamiento', 'descripcion': 'Modelo masivo de logica y resolucion analitica profunda.'}, {'id': 'nvidia/nemotron-3.5-lightning:free', 'nombre': 'NVIDIA Nemotron 3.5 Lightning Free', 'contexto': 1000000, 'categoria': 'gran_contexto', 'descripcion': '1 Millon de tokens de contexto con alta velocidad de inferencia.'}, {'id': 'nvidia/nemotron-3-ultra-550b-a55b:free', 'nombre': 'NVIDIA Nemotron 3 Ultra 550B Free', 'contexto': 1000000, 'categoria': 'pesado', 'descripcion': 'Arquitectura MoE de maximo parametro con coste cero.'}, {'id': 'nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free', 'nombre': 'NVIDIA Nemotron 3 Nano Reasoning Free', 'contexto': 256000, 'categoria': 'razonamiento', 'descripcion': 'Razonamiento por pasos y logica formal.'}, {'id': 'cohere/north-mini-code:free', 'nombre': 'Cohere North Mini Code Free', 'contexto': 256000, 'categoria': 'codigo', 'descripcion': 'Especializado en generacion, refactorizacion y sintaxis de codigo.'}, {'id': 'z-ai/glm-5.2:free', 'nombre': 'Z-AI GLM 5.2 Free', 'contexto': 32768, 'categoria': 'general', 'descripcion': 'Conversacion rapida y concisa con baja utilizacion de memoria.'}, {'id': 'liquid/lfm-2.5-2.6b:free', 'nombre': 'Liquid LFM 2.5 2.6B Free', 'contexto': 65536, 'categoria': 'eficiente', 'descripcion': 'Red neuronal de estado liquido con generacion ultrarrapida.'}, {'id': 'thinkingmachines/inkling:free', 'nombre': 'ThinkingMachines Inkling Free', 'contexto': 1048576, 'categoria': 'gran_contexto', 'descripcion': 'Contexto extendido de 1M tokens para analisis de grandes textos.'}, {'id': 'thinkingmachines/inkling-small:free', 'nombre': 'ThinkingMachines Inkling Small Free', 'contexto': 1048576, 'categoria': 'gran_contexto', 'descripcion': 'Version compacta optimizada para respuestas inmediatas.'}, {'id': 'poolside/laguna-s-2.1:free', 'nombre': 'Poolside Laguna S 2.1 Free', 'contexto': 262144, 'categoria': 'codigo', 'descripcion': 'Optimizacion de nivel produccion para arquitectura de software.'}, {'id': 'poolside/laguna-xs-2.1:free', 'nombre': 'Poolside Laguna XS 2.1 Free', 'contexto': 262144, 'categoria': 'codigo', 'descripcion': 'Asistencia ultraligera para programacion interactiva.'}, {'id': 'dots-studio/dots-3-note-preview:free', 'nombre': 'Dots 3 Note Preview Free', 'contexto': 512000, 'categoria': 'analisis', 'descripcion': '512k tokens dedicados a sintesis y notas estructuradas.'}, {'id': 'nex-agi/nex-n2.5-mini:free', 'nombre': 'Nex-AGI N2.5 Mini Free', 'contexto': 262144, 'categoria': 'general', 'descripcion': 'Respuestas agiles para dialogos de soporte continuo.'}, {'id': 'nex-agi/nex-n2.5-pro:free', 'nombre': 'Nex-AGI N2.5 Pro Free', 'contexto': 262144, 'categoria': 'razonamiento', 'descripcion': 'Capacidades avanzadas de comprension logica.'}, {'id': 'inclusionai/ling-3.0-flash-vl:free', 'nombre': 'InclusionAI Ling 3.0 Flash VL Free', 'contexto': 262144, 'categoria': 'multimodal', 'descripcion': 'Comprension multimodal y razonamiento veloz.'}, {'id': 'inclusionai/ling-3.0-flash-fin:free', 'nombre': 'InclusionAI Ling 3.0 Flash Fin Free', 'contexto': 262144, 'categoria': 'analisis', 'descripcion': 'Especializado en analitica cuantitativa y calculos precisos.'}, {'id': 'inclusionai/ling-3.0-flash-sante:free', 'nombre': 'InclusionAI Ling 3.0 Flash Sante Free', 'contexto': 262144, 'categoria': 'general', 'descripcion': 'Alta fidelidad y alineacion etica de respuestas.'}]
...
class GestorModelosGratuitos:
    ...
    def rotar_al_siguiente_modelo(self, razon: str='error') -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            logger.warning('Rotacion de modelo gratuito: %s -> %s (Razon: %s)', actual, nuevo['id'], razon)
            return nuevo
            ...
            with urllib.request.urlopen(req, timeout=8.0) as resp:
                ...
                for item in data.get('data', []):
                    ...
                    if (':free' in m_id or (p_cost == 0.0 and c_cost == 0.0)) and m_id not in existentes:
                        nuevo_m = {'id': m_id, 'nombre': item.get('name', m_id), 'contexto': item.get('context_length', 262144), 'categoria': 'descubierto', 'descripcion': item.get('description', 'Modelo gratuito descubierto en linea.')[:120]}
                        with self._lock:
...
class ClienteIAFree:
    ...
    def generar_respuesta(self, prompt: str, contexto_neuronal: Optional[Dict[str, Any]]=None, max_reintentos: int=4, stream_en_vivo: bool=True) -> Tuple[str, str, float]:
        """
        ...
        prompt_sistema = f"Eres LucIA, el sistema cognitivo distribuido de WoldVirtualP2P3D (2026). Tu tono neuronal actual es '{tono}' y tu valencia es {valencia:+.2f}. Responde de forma clara, directa, tecnica y cordial a la solicitud del usuario."
        intentos = 0
        while intentos < max_reintentos:
            modelo_actual = self.gestor.obtener_modelo_activo()
            modelo_id = modelo_actual['id']
            t_inicio = time.perf_counter()
            payload = json.dumps({'model': modelo_id, 'messages': [{'role': 'system', 'content': prompt_sistema}, {'role': 'user', 'content': prompt}], 'temperature': self.temperatura, 'max_tokens': self.max_tokens, 'stream': stream_en_vivo}).encode('utf-8')
            headers = {'Authorization': f'Bearer {self.gestor.api_key}', 'Content-Type': 'application/json', 'HTTP-Referer': 'https://woldvirtualp2p3d.network', 'X-Title': 'LucIA-IAFREE-Subsystem'}
            ...
            try:
                with urllib.request.urlopen(req, timeout=25.0) as respuesta:
                    if stream_en_vivo:
                        sys.stdout.write(f'\n\x1b[96m  LucIA [Free:{modelo_id}] ▶ \x1b[0m')
                        sys.stdout.flush()
                ...
                intentos += 1
                time.sleep(0.4)
        return ('[IAFREE] Todos los modelos gratuitos consultados se encuentran temporalmente saturados. Activando reflejo neuronal interno autónomo.', 'fallback_agotado', 0.0)

    def obtener_metricas_consumo(self) -> Dict[str, Any]:
        """Retorna un reporte de auditoria demostrando coste total de $0.00 USD."""
        with self._lock:
            return {'consultas_totales_gratuitas': self.total_consultas_gratuitas, 'caracteres_recibidos': self.total_caracteres_recibidos, 'costo_acumulado_usd': 0.0, 'modelo_activo': self.gestor.obtener_modelo_activo()['id'], 'total_modelos_en_pool': len(self.gestor._modelos), 'fallos_registrados': dict(self.gestor._fallos_consecutivos)}
    ...
    def benchmark_rapido_modelos(self, max_modelos: int=3) -> Dict[str, float]:
        ...
        for m in self.gestor.listar_modelos()[:max_modelos]:
            ...
            try:
                payload = json.dumps({'model': m_id, 'messages': [{'role': 'user', 'content': '1+1='}], 'max_tokens': 4}).encode('utf-8')
                req = urllib.request.Request(self.ENDPOINT_CHAT, data=payload, headers={'Authorization': f'Bearer {self.gestor.api_key}', 'Content-Type': 'application/json'}, method='POST')
...
if __name__ == '__main__':
    print('\n\x1b[1;36m' + '=' * 74 + '\x1b[0m')
    print(f'  \x1b[1;32mSUBSISTEMA DE INFERENCIA GRATUITA -- {__subsystem__} v{__version__}\x1b[0m')
    print('\x1b[1;36m' + '=' * 74 + '\x1b[0m')
    cliente = get_cliente_iafree()
    print(f'  Autenticado en OpenRouter : \x1b[1;33m{cliente.esta_autenticado()}\x1b[0m')
    print(f'  Modelos :free precargados : \x1b[1;35m{len(CATALOGO_MODELOS_GRATUITOS)}\x1b[0m')
    print('  Actualizando modelos en vivo desde OpenRouter API...')
    total = cliente.gestor.actualizar_catalogo_online()
    print(f'  Total modelos :free listos: \x1b[1;32m{total}\x1b[0m\n')
    print('\x1b[1;37mModelos Gratuitos Principales (Coste $0.00):\x1b[0m')
    for m in cliente.gestor.listar_modelos()[:8]:
        print(f"  * \x1b[1;36m{m['id']}\x1b[0m | {m['contexto']} tok | {m['nombre']}")
    print('\n\x1b[1;32mRealizando prueba de inferencia a coste cero ($0.00)...\x1b[0m')
    resp, mod, lat = cliente.generar_respuesta('Hola LucIA, confirma que este subsistema de inferencia es 100% gratuito.', contexto_neuronal={'tono_cognitivo': 'seguro y analitico', 'estado_emocional': 0.45}, stream_en_vivo=True)
    print(f'\n  Modelo utilizado: \x1b[1;33m{mod}\x1b[0m | Latencia: \x1b[1;32m{lat:.0f}ms\x1b[0m')
    auditoria = cliente.obtener_metricas_consumo()
    print(f"  Auditoria de costo : \x1b[1;32m${auditoria['costo_acumulado_usd']:.2f} USD\x1b[0m")
    print('\x1b[1;36m' + '=' * 74 + '\x1b[0m\n')

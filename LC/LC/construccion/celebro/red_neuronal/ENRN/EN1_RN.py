...
class NeuronaEntradaBasica(NeuronaEntradaBase):
    ...
    def __init__(self, input_size: int, output_size: int, nombre: str='NeuronaEntradaBasica', factor_escala: float=1.0, learning_rate: float=0.001, optimizador: str='adam', beta1: float=0.9, beta2: float=0.999, epsilon: float=1e-08, weight_decay: float=0.0, grad_clip: float=5.0, lr_decay: float=0.0, lr_min: float=1e-06, usar_cache_forward: bool=True, normalizar_entrada: bool=False, semilla: Optional[int]=None, capacidad_historial: int=256):
    ...
    def backward(self, gradiente_salida: np.ndarray, entrada: Optional[np.ndarray]=None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self.historial_gradientes.append({'pesos': np.asarray(grad_pesos, dtype=_dtype()).copy(), 'sesgo': np.asarray(grad_sesgo, dtype=_dtype()).copy(), 'norma_pesos': float(np.linalg.norm(grad_pesos)), 'norma_sesgo': float(np.linalg.norm(grad_sesgo))})
    ...
    def obtener_estadisticas(self) -> Dict[str, Any]:
        base = {'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'activaciones': len(self.historial_activaciones)}
        base.update({'forwards': self.contador_forward, 'backwards': self.contador_backward, 'lr_actual': self.lr_actual, 'paso': self._paso, 'optimizador': self.optimizador})
        return base
    ...
    def obtener_estadisticas_completas(self) -> Dict[str, Any]:
        ...
        stats.update({'varianza_esperada': self.varianza_esperada, 'varianza_real': self.varianza_real, 'media_abs_pesos': pw, 'norma_pesos': nw, 'mejor_loss': self.mejor_loss, 'ultima_loss': self.historial_loss[-1] if self.historial_loss else None, 't_medio_forward_ms': tf * 1000.0, 't_medio_backward_ms': tb * 1000.0})
        return stats
    ...
    def resumen_rendimiento(self) -> Dict[str, Any]:
        return {'forwards': self.contador_forward, 'backwards': self.contador_backward, 't_forward_total_s': self.tiempo_forward_acum, 't_backward_total_s': self.tiempo_backward_acum, 't_medio_forward_ms': self.tiempo_forward_acum / max(1, self.contador_forward) * 1000.0, 't_medio_backward_ms': self.tiempo_backward_acum / max(1, self.contador_backward) * 1000.0}
    ...
    def cargar(cls, ruta: str) -> 'NeuronaEntradaBasica':
        with open(ruta, 'r', encoding='utf-8') as f:
            d = json.load(f)
        obj = cls(d['input_size'], d['output_size'], d.get('nombre', 'NeuronaEntradaBasica'), factor_escala=d.get('factor_escala', 1.0), learning_rate=d.get('lr_base', 0.001), optimizador=d.get('optimizador', 'adam'))

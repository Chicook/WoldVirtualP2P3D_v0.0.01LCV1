...
class NeuronaEntradaBatchNorm(NeuronaEntradaBase):
    ...
    def __init__(self, input_size: int, output_size: int, nombre: str='NeuronaEntradaBatchNorm', momentum: float=0.9, epsilon: float=1e-05, factor_escala: float=1.0, modo_entrenamiento: bool=True, learning_rate: float=0.001, optimizador: str='adam', beta1: float=0.9, beta2: float=0.999, eps_adam: float=1e-08, weight_decay: float=0.0, grad_clip: float=5.0, lr_decay: float=0.0, lr_min: float=1e-06, usar_cache_forward: bool=True, semilla: Optional[int]=None, capacidad_historial: int=256):
    ...
    def _bn_backward(self, g: np.ndarray, cache: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gz = 1.0 / m * gamma * std_inv * (m * g64 - np.sum(g64, axis=0, keepdims=True) - zn * np.sum(g64 * zn, axis=0, keepdims=True))
    ...
    def backward(self, gradiente_salida: np.ndarray, entrada: Optional[np.ndarray]=None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if bn is None:
            gz = g
            g_gamma = np.zeros_like(self.gamma)
            g_beta = np.zeros_like(self.beta)
        else:
    ...
    def obtener_estadisticas_completas(self) -> Dict[str, Any]:
        ...
        stats.update({'varianza_real': self.varianza_real, 'momentum': self.momentum, 'running_mean_norm': float(np.linalg.norm(self.running_mean)) if self.running_mean is not None else 0.0, 'running_var_media': float(np.mean(self.running_var)) if self.running_var is not None else 0.0, 'gamma_media': float(np.mean(self.gamma)) if self.gamma is not None else 0.0, 'mejor_loss': self.mejor_loss, 'ultima_loss': self.historial_loss[-1] if self.historial_loss else None, 't_medio_forward_ms': self.tiempo_forward_acum / max(1, self.contador_forward) * 1000.0, 't_medio_backward_ms': self.tiempo_backward_acum / max(1, self.contador_backward) * 1000.0})
        return stats
    ...
    def a_dict(self) -> Dict[str, Any]:
        return {'input_size': self.input_size, 'output_size': self.output_size, 'nombre': self.nombre, 'momentum': self.momentum, 'lr_base': self.lr_base, 'optimizador': self.optimizador, 'pesos': self.pesos.tolist() if self.pesos is not None else None, 'sesgo': self.sesgo.tolist() if self.sesgo is not None else None, 'gamma': self.gamma.tolist() if self.gamma is not None else None, 'beta': self.beta.tolist() if self.beta is not None else None}
    ...
    def cargar(cls, ruta: str) -> 'NeuronaEntradaBatchNorm':
        with open(ruta, 'r', encoding='utf-8') as f:
            d = json.load(f)
        obj = cls(d['input_size'], d['output_size'], d.get('nombre', 'NeuronaEntradaBatchNorm'), momentum=d.get('momentum', 0.9))

...
class NeuronaEntradaLeCun(NeuronaEntradaBase):
    ...
    def __init__(self, input_size: int, output_size: int, nombre: str='NeuronaEntradaLeCun', usar_uniforme: bool=False, factor_escala: float=1.0, learning_rate: float=0.001, optimizador: str='adam', beta1: float=0.9, beta2: float=0.999, epsilon: float=1e-08, weight_decay: float=0.0, grad_clip: float=5.0, lr_decay: float=0.0, lr_min: float=1e-06, usar_cache_forward: bool=True, normalizar_entrada: bool=False, semilla: Optional[int]=None, capacidad_historial: int=256):
    ...
    def cargar(cls, ruta: str) -> 'NeuronaEntradaLeCun':
        with open(ruta, 'r', encoding='utf-8') as f:
            d = json.load(f)
        obj = cls(d['input_size'], d['output_size'], d.get('nombre', 'NeuronaEntradaLeCun'), usar_uniforme=d.get('usar_uniforme', False), factor_escala=d.get('factor_escala', 1.0), learning_rate=d.get('lr_base', 0.001), optimizador=d.get('optimizador', 'adam'))

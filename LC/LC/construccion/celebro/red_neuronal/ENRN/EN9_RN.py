...
class NeuronaEntradaTanh(NeuronaEntradaBase):
    """EN9 Tanh eficiente: Xavier init, derivada 1-t^2, Adam/SGD.

    Preserva API: factor_escala, saturacion, inicializar_pesos,
    forward, backward, estadisticas, estabilidad.
    """

    def __init__(self, input_size: int, output_size: int, nombre: str='NeuronaEntradaTanh', factor_escala: float=1.0, umbral_saturacion: float=0.95, fraccion_saturacion: float=0.1, learning_rate: float=0.001, optimizador: str='adam', beta1: float=0.9, beta2: float=0.999, epsilon: float=1e-08, weight_decay: float=0.0, grad_clip: float=5.0, lr_decay: float=0.0, lr_min: float=1e-06, usar_cache_forward: bool=True, semilla: Optional[int]=None, capacidad_historial: int=256):
        super().__init__(input_size, output_size, nombre)
        self.factor_escala = float(factor_escala)
        self.umbral_saturacion = float(umbral_saturacion)
        self.fraccion_saturacion = float(fraccion_saturacion)
        self.varianza_real = 0.0
        self.varianza_esperada = 1.0 / ((input_size + output_size) / 2.0)
        self.saturacion = False
        self.ratio_saturacion = 0.0
    ...
    def _chequear_saturacion(self, salida: np.ndarray) -> float:
        ratio = float(np.sum(np.abs(salida) > self.umbral_saturacion)) / max(1, salida.size)
        self.ratio_saturacion = 0.9 * self.ratio_saturacion + 0.1 * ratio if self.historial_sat else ratio
        self.saturacion = bool(self.ratio_saturacion > self.fraccion_saturacion)
        self.historial_sat.append(ratio)
        return ratio
    ...
    def forward(self, entrada: np.ndarray) -> np.ndarray:
        ...
        salida = np.tanh(suma).astype(_dtype(), copy=False)
        salida = np.ascontiguousarray(salida, dtype=_dtype())
        self._chequear_saturacion(salida)
    ...
    def backward(self, gradiente_salida: np.ndarray, entrada: Optional[np.ndarray]=None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if g.ndim == 1:
            g = g.reshape(1, -1)
        t = None
        if entrada is None and 'salida' in self._cache:
            t = np.asarray(self._cache['salida'], dtype=np.float64)
        elif self.historial_activaciones.to_list():
            t = np.asarray(self.historial_activaciones.to_list()[-1], dtype=np.float64)
        if t is not None and t.shape == g.shape:
            g = g * (1.0 - t ** 2)
    ...
    def grad_entrada(self, gradiente_salida: np.ndarray) -> np.ndarray:
        ...
        if g.ndim == 1:
            g = g.reshape(1, -1)
        t = self._cache.get('salida')
        if t is not None and np.asarray(t).shape == g.shape:
            g = g * (1.0 - np.asarray(t, dtype=np.float64) ** 2)
    ...
    def entrenar_paso(self, entrada: np.ndarray, objetivo: np.ndarray, en_rango_tanh: bool=True) -> Dict[str, float]:
        ...
        err = (pred.astype(np.float64) - y) / max(1, pred.shape[0])
        loss = float(np.mean((pred.astype(np.float64) - y) ** 2))
    ...
    def obtener_estadisticas_completas(self) -> Dict[str, Any]:
        ...
        stats.update({'varianza_real': self.varianza_real, 'saturacion': self.saturacion, 'ratio_saturacion': self.ratio_saturacion, 'mejor_loss': self.mejor_loss, 'ultima_loss': self.historial_loss[-1] if self.historial_loss else None, 't_medio_forward_ms': self.tiempo_forward_acum / max(1, self.contador_forward) * 1000.0, 't_medio_backward_ms': self.tiempo_backward_acum / max(1, self.contador_backward) * 1000.0})
        return stats
    ...
    @classmethod
    def cargar(cls, ruta: str) -> 'NeuronaEntradaTanh':
        with open(ruta, 'r', encoding='utf-8') as f:
            d = json.load(f)
        obj = cls(d['input_size'], d['output_size'], d.get('nombre', 'NeuronaEntradaTanh'))
    ...
    def __str__(self) -> str:
        return f'{self.nombre}(ent={self.input_size},sal={self.output_size},act=Tanh,opt={self.optimizador})'

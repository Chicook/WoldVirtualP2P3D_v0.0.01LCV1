...
class NeuronaRefuerzoActorCritic(NeuronaRefuerzoBase):
    ...
    def __init__(self, input_size: int, output_size: int, nombre: str='NeuronaRefuerzoActorCritic', learning_rate_actor: float=0.001, learning_rate_critic: float=0.002, gamma: float=0.99, lambda_gae: float=0.95, usar_espectral: bool=True, meta_learning: bool=True, grad_clip: float=5.0, semilla: int=42, capacidad_historial: int=512):
        ...
        self.tiempo_fwd = 0.0
        self.tiempo_upd = 0.0
        self.estadisticas_ac = {'convergencia_actor': 0.0, 'convergencia_critic': 0.0, 'estabilidad_actor': 0.0, 'estabilidad_critic': 0.0, 'advantage_media': 0.0, 'advantage_std': 0.0, 'valor_error_medio': 0.0, 'policy_loss_medio': 0.0, 'meta_adaptaciones': 0, 'espectral_updates': 0}
    ...
    def calcular_gae_advantages(self, recompensas: List[float], valores: List[float], siguiente_valor: float=0.0) -> np.ndarray:
    ...
    def calcular_gradientes_ac(self, estados: List[np.ndarray], acciones: List[int], advantages: List[float], valores_objetivo: List[float]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        pl = float(np.mean(-np.log(probs[np.arange(n), A] + 1e-08) * Adv))
    ...
    def actualizar_pesos(self, grad_actor_pesos: np.ndarray, grad_actor_sesgo: np.ndarray, grad_critic_pesos: np.ndarray, grad_critic_sesgo: np.ndarray) -> Dict[str, float]:
        t0 = time.perf_counter()
    ...
    def entrenar_paso(self, estados: List[np.ndarray], acciones: List[int], recompensas: List[float], valores: List[float]) -> Dict[str, float]:
    ...
    def _adaptar_meta_learning(self) -> None:
        ...
        self.learning_rate_actor = float(np.clip(self.learning_rate_actor * (0.95 if ta > 0 else 1.05), 1e-06, 0.01))
        self.learning_rate_critic = float(np.clip(self.learning_rate_critic * (0.95 if tc > 0 else 1.05), 1e-06, 0.01))
    ...
    def obtener_estadisticas_ac(self) -> Dict[str, Any]:
        ...
        stats: Dict[str, Any] = {'learning_rate_actor': self.learning_rate_actor, 'learning_rate_critic': self.learning_rate_critic, 'gamma': self.gamma, 'lambda_gae': self.lambda_gae, 'convergencia_actor': self.estadisticas_ac['convergencia_actor'], 'convergencia_critic': self.estadisticas_ac['convergencia_critic'], 'estabilidad_actor': ea, 'estabilidad_critic': ec, 'meta_adaptaciones': self.estadisticas_ac['meta_adaptaciones'], 'espectral_updates': self.estadisticas_ac['espectral_updates'], 'usar_espectral': self.usar_espectral, 'meta_learning': self.meta_learning, 't_medio_fwd_us': self.tiempo_fwd / max(1, self.pasos + 1) * 1000000.0, 't_medio_upd_us': self.tiempo_upd / max(1, self.pasos) * 1000000.0}
        if len(self.historial_advantages):
            ...
            stats.update({'advantage_media': float(arr.mean()), 'advantage_std': float(arr.std()), 'advantage_min': float(arr.min()), 'advantage_max': float(arr.max())})
    ...
    def verificar_estabilidad(self) -> Dict[str, bool]:
        ea, ec = self.calcular_estabilidad_ac()
        est = {'actor_convergencia_ok': self.estadisticas_ac['convergencia_actor'] > 0.7, 'critic_convergencia_ok': self.estadisticas_ac['convergencia_critic'] > 0.7, 'actor_estable': ea > 0.7, 'critic_estable': ec > 0.7, 'lr_actor_ok': 1e-06 <= self.learning_rate_actor <= 0.01, 'lr_critic_ok': 1e-06 <= self.learning_rate_critic <= 0.01}
    ...
    def reinicializar_con_parametros(self, learning_rate_actor: float=None, learning_rate_critic: float=None, gamma: float=None, lambda_gae: float=None) -> None:
    ...
    def a_dict(self) -> Dict[str, Any]:
        return {'input_size': self.input_size, 'output_size': self.output_size, 'nombre': self.nombre, 'lr_actor': self.learning_rate_actor, 'lr_critic': self.learning_rate_critic, 'pesos_actor': self.pesos_actor.tolist() if self.pesos_actor is not None else None, 'pesos_critic': self.pesos_critic.tolist() if self.pesos_critic is not None else None}
    ...
    def __str__(self) -> str:
        return f'NeuronaRefuerzoActorCritic(entrada={self.input_size}, salida={self.output_size}, lr_actor={self.learning_rate_actor}, lr_critic={self.learning_rate_critic}, espectral={self.usar_espectral})'

    def __repr__(self) -> str:
        return self.__str__()

def crear_neurona_actor_critic(input_size: int, output_size: int, configuracion: Dict[str, Any]=None) -> NeuronaRefuerzoActorCritic:
    configuracion = configuracion or {}
    return NeuronaRefuerzoActorCritic(input_size=input_size, output_size=output_size, nombre=configuracion.get('nombre', 'NeuronaRefuerzoActorCritic'), learning_rate_actor=configuracion.get('learning_rate_actor', 0.001), learning_rate_critic=configuracion.get('learning_rate_critic', 0.002), gamma=configuracion.get('gamma', 0.99), lambda_gae=configuracion.get('lambda_gae', 0.95), usar_espectral=configuracion.get('usar_espectral', True), meta_learning=configuracion.get('meta_learning', True))
...
RFEN3_CONFIG = {'inicializacion_preferida': 'espectral', 'learning_rate_actor_default': 0.001, 'learning_rate_critic_default': 0.002, 'gamma_default': 0.99, 'lambda_gae_default': 0.95, 'meta_learning_default': True, 'espectral_default': True, 'umbral_convergencia': 0.7, 'umbral_estabilidad': 0.7}

...
LUCIA_RL_CONFIG = {'precision': 'float32', 'random_seed': 42, 'default_learning_rate': 0.1}
...
class NeuronaRefuerzoQLearning(NeuronaRefuerzoBase):
    ...
    def __init__(self, input_size: int, output_size: int, nombre: str='NeuronaRefuerzoQLearning', learning_rate: float=0.1, gamma: float=0.99, epsilon: float=0.1, epsilon_decay: float=0.995, epsilon_min: float=0.01, usar_xavier: bool=True, usar_doble_q: bool=False, sync_objetivo_cada: int=0, semilla: int=42, capacidad_historial: int=512):
        ...
        self.tiempo_fwd = 0.0
        self.tiempo_upd = 0.0
        self.estadisticas_qlearning = {'convergencia_q': 0.0, 'estabilidad_q': 0.0, 'exploracion_rate': 0, 'exploitacion_rate': 0, 'q_values_media': 0.0, 'q_values_std': 0.0, 'q_values_min': 0.0, 'q_values_max': 0.0, 'actualizaciones_q': 0, 'convergencia_detectada': False}
    ...
    def actualizar_q_value(self, estado: int, accion: int, recompensa: float, siguiente_estado: int, terminado: bool=False) -> float:
        t0 = time.perf_counter()
        self._asegurar()
    ...
    def actualizar_q_batch(self, estados: np.ndarray, acciones: np.ndarray, recompensas: np.ndarray, siguientes: np.ndarray, terminados: np.ndarray) -> float:
    ...
    def obtener_estadisticas_q(self) -> Dict[str, Any]:
        ...
        return {'q_values_media': float(np.mean(qf)), 'q_values_std': float(np.std(qf)), 'q_values_min': float(np.min(qf)), 'q_values_max': float(np.max(qf)), 'epsilon_actual': self.epsilon, 'learning_rate': self.learning_rate, 'gamma': self.gamma, 'convergencia_q': self.estadisticas_qlearning['convergencia_q'], 'estabilidad_q': self.calcular_estabilidad_q(), 'actualizaciones_q': self.estadisticas_qlearning['actualizaciones_q'], 'convergencia_detectada': self.estadisticas_qlearning['convergencia_detectada'], 'tasa_exploracion': self.estadisticas_qlearning['exploracion_rate'] / max(1, tot), 'tasa_exploitacion': self.estadisticas_qlearning['exploitacion_rate'] / max(1, tot), 't_medio_fwd_us': self.tiempo_fwd / max(1, self.pasos) * 1000000.0, 't_medio_upd_us': self.tiempo_upd / max(1, self.pasos) * 1000000.0}

    def verificar_estabilidad(self) -> Dict[str, bool]:
        est = {'convergencia_ok': self.estadisticas_qlearning['convergencia_q'] > 0.8, 'q_values_estables': self.calcular_estabilidad_q() > 0.7, 'epsilon_apropiado': self.epsilon_min <= self.epsilon <= 1.0}
        if self.q_table is not None:
            ...
            est.update({'q_values_no_explosivos': True, 'q_values_no_desaparecen': True, 'sin_nan': True, 'sin_inf': True})
        return est

    def reinicializar_con_parametros(self, learning_rate: float=None, gamma: float=None, epsilon: float=None, epsilon_decay: float=None, epsilon_min: float=None) -> None:
    ...
    def resumen_rendimiento(self) -> Dict[str, Any]:
        return {'pasos': self.pasos, 't_fwd_s': self.tiempo_fwd, 't_upd_s': self.tiempo_upd, 't_medio_fwd_us': self.tiempo_fwd / max(1, self.pasos) * 1000000.0, 't_medio_upd_us': self.tiempo_upd / max(1, self.pasos) * 1000000.0, 'actualizaciones': self.estadisticas_qlearning['actualizaciones_q']}

    def a_dict(self) -> Dict[str, Any]:
        return {'input_size': self.input_size, 'output_size': self.output_size, 'nombre': self.nombre, 'learning_rate': self.learning_rate, 'gamma': self.gamma, 'epsilon': self.epsilon, 'epsilon_decay': self.epsilon_decay, 'epsilon_min': self.epsilon_min, 'q_table': self.q_table.tolist() if self.q_table is not None else None}
    ...
    def cargar(cls, ruta: str) -> 'NeuronaRefuerzoQLearning':
        ...
        with open(ruta, 'r', encoding='utf-8') as f:
            d = json.load(f)
        o = cls(d['input_size'], d['output_size'], d.get('nombre', 'NeuronaRefuerzoQLearning'), learning_rate=d.get('learning_rate', 0.1), gamma=d.get('gamma', 0.99), epsilon=d.get('epsilon', 0.1))
    ...
    def __str__(self) -> str:
        return f'NeuronaRefuerzoQLearning(entrada={self.input_size}, salida={self.output_size}, lr={self.learning_rate}, gamma={self.gamma}, epsilon={self.epsilon:.3f})'

    def __repr__(self) -> str:
        return self.__str__()

def crear_neurona_qlearning(input_size: int, output_size: int, configuracion: Dict[str, Any]=None) -> NeuronaRefuerzoQLearning:
    configuracion = configuracion or {}
    return NeuronaRefuerzoQLearning(input_size=input_size, output_size=output_size, nombre=configuracion.get('nombre', 'NeuronaRefuerzoQLearning'), learning_rate=configuracion.get('learning_rate', 0.1), gamma=configuracion.get('gamma', 0.99), epsilon=configuracion.get('epsilon', 0.1), epsilon_decay=configuracion.get('epsilon_decay', 0.995), epsilon_min=configuracion.get('epsilon_min', 0.01), usar_xavier=configuracion.get('usar_xavier', True), usar_doble_q=configuracion.get('usar_doble_q', False))

def analizar_convergencia_qlearning(neurona: NeuronaRefuerzoQLearning) -> Dict[str, Any]:
    return {'convergencia_q': neurona.estadisticas_qlearning['convergencia_q'], 'estabilidad_q': neurona.calcular_estabilidad_q(), 'epsilon_actual': neurona.epsilon, 'actualizaciones_q': neurona.estadisticas_qlearning['actualizaciones_q'], 'convergencia_detectada': neurona.estadisticas_qlearning['convergencia_detectada'], 'estadisticas_q': neurona.obtener_estadisticas_q(), 'estabilidad_verificada': neurona.verificar_estabilidad()}

def comparar_inicializaciones_qlearning(input_size: int, output_size: int, episodios: int=1000) -> Dict[str, Any]:
    ...
    for k, n in (('xavier', nx), ('ceros', nc)):
        out[k] = {'convergencia_q': n.estadisticas_qlearning['convergencia_q'], 'estabilidad_q': n.calcular_estabilidad_q(), 'q_values_media': float(np.mean(n.q_table)), 'q_values_std': float(np.std(n.q_table)), 'actualizaciones_q': n.estadisticas_qlearning['actualizaciones_q']}
    out['ratios'] = {'convergencia_q': out['xavier']['convergencia_q'] / max(1e-12, out['ceros']['convergencia_q']), 'estabilidad_q': out['xavier']['estabilidad_q'] / max(1e-12, out['ceros']['estabilidad_q'])}
    return out
RFEN1_CONFIG = {'inicializacion_preferida': 'xavier', 'learning_rate_default': 0.1, 'gamma_default': 0.99, 'epsilon_default': 0.1, 'epsilon_decay_default': 0.995, 'epsilon_min_default': 0.01, 'umbral_convergencia': 0.95, 'umbral_estabilidad': 0.7}

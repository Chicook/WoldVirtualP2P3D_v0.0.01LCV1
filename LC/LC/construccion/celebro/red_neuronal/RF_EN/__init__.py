...
class NeuronaRefuerzoBase:
    ...
    def add_historial_activacion(self, activacion: np.ndarray) -> None:
        self.historial_activaciones.append(activacion.copy())

    def add_historial_gradiente(self, gradiente: Dict[str, np.ndarray]) -> None:
        self.historial_gradientes.append({k: v.copy() for k, v in gradiente.items()})

def calcular_recompensa_descontada(recompensas: List[float], gamma: float) -> np.ndarray:
    n = len(recompensas)
    discounted = np.zeros(n, dtype=np.float64)
    acum = 0.0
    for t in range(n - 1, -1, -1):
        acum = recompensas[t] + gamma * acum
        discounted[t] = acum
    return discounted
...
def normalizar_recompensas(recompensas: np.ndarray) -> np.ndarray:
    ...
    return (recompensas - media) / max(1e-08, std)
...
def calcular_gae(recompensas: List[float], valores: List[float], gamma: float=0.99, lambda_gae: float=0.95) -> np.ndarray:
    ...
    for t in range(n - 1, -1, -1):
        ...
        adv = delta + gamma * lambda_gae * adv
        advantages[t] = adv
    return advantages
...
def aplicar_clip_gradientes(gradiente: np.ndarray, max_norm: float=0.5) -> np.ndarray:
    ...
    if norma > max_norm:
        return gradiente * max_norm / (norma + 1e-08)
    return gradiente

def crear_buffer_experiencia(capacidad: int=10000) -> deque:
    return deque(maxlen=capacidad)

def muestrear_buffer(buffer: deque, tamano: int) -> List[Any]:
    return random.sample(list(buffer), min(tamano, len(buffer)))
...
def calcular_loss(y_pred: np.ndarray, y_target: np.ndarray) -> float:
    return float(np.mean((y_pred - y_target) ** 2))

def crear_buffer_priorizado(capacidad: int=10000, alpha: float=0.6) -> Dict:
    return {'buffer': deque(maxlen=capacidad), 'prioridades': deque(maxlen=capacidad), 'alpha': alpha}

def muestrear_priorizado(buffer: Dict, tamano: int, beta: float=0.4) -> List[Any]:
    buf = buffer['buffer']
    pri = buffer['prioridades']
    if len(buf) < tamano:
        return []
    pa = np.array(list(pri))
    pb = pa ** buffer['alpha']
    pb /= np.sum(pb)
...
def actualizar_prioridades(buffer: Dict, indices: List[int], errores: np.ndarray, beta: float=0.4) -> None:
    for idx, err in zip(indices, errores):
        if idx < len(buffer['prioridades']):
            buffer['prioridades'][idx] = (abs(err) + 1e-06) ** beta

def compute_returns(recompensas: List[float], gamma: float) -> np.ndarray:
    return calcular_recompensa_descontada(recompensas, gamma)

def discount_cumulative_returns(recompensas: List[float], gamma: float) -> np.ndarray:
    return calcular_recompensa_descontada(recompensas, gamma)
...
def mean_std_normalize(data: np.ndarray) -> Tuple[np.ndarray, float, float]:
    ...
    return ((data - media) / max(1e-08, std), media, std)

def sample_gumbel_softmax(logits: np.ndarray, temperature: float=1.0) -> np.ndarray:
    gumbel = -np.log(-np.log(np.random.uniform(1e-08, 1.0, size=logits.shape)))
    y = logits + gumbel
    y /= max(1e-12, temperature)
...
def lr_scheduler(step: int, initial_lr: float, decay: float=0.99) -> float:
    return initial_lr * decay ** step

def early_stopping_check(historial: List[float], paciencia: int=20, umbral: float=0.0001) -> bool:
    if len(historial) < paciencia + 5:
        return False
    reciente = historial[-paciencia:]
    mejor = min(reciente)
    mejor_idx = reciente.index(mejor)
    if paciencia - mejor_idx > paciencia * 0.8:
        return True
    var = np.var(reciente)
    return var < umbral

def neuron_info(neurona) -> Dict[str, Any]:
    return {'nombre': neurona.nombre, 'input_size': neurona.input_size, 'output_size': neurona.output_size, 'pasos': neurona.pasos, 'params': count_parameters(neurona), 'historial_len': len(neurona.historial_gradientes)}
...
def train_epoch(neurona, dataset: List, batch_size: int=32, max_norm: float=0.5) -> float:
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        result = train_batch_neuron(neurona, batch, max_norm)
        if result is not None:
            total_loss += result
            n_batches += 1
    return total_loss / max(1, n_batches)

def parallel_train(neuronas: List, dataset: List, batch_size: int=32) -> List[float]:
    return [train_epoch(n, dataset, batch_size) for n in neuronas]
...
def train_batch_neuron(neurona, batch: List[Dict[str, Any]], max_norm: float=0.5) -> Optional[float]:
    if hasattr(neurona, 'calcular_gradientes_rainbow'):
        gw, gs = neurona.calcular_gradientes_rainbow(batch)
        gw = aplicar_clip_gradientes(gw, max_norm)
        gs = aplicar_clip_gradientes(gs, max_norm)
        neurona.actualizar_pesos(gw, gs)
    elif hasattr(neurona, 'calcular_gradientes_impala'):
        gw = neurona.calcular_gradientes_impala(batch, 0)
        neurona.actualizar_pesos_learner(*gw)
    else:
        return None
...
def evaluate_neurona(neurona, estados_test, acciones_test, recompensas_test) -> Dict[str, float]:
    ...
    if preds_2d.shape[1] > 1:
        ...
        acc = 0.0
        mse = float(np.mean((preds_2d.flatten() - recompensas_test) ** 2))
...
def analizar_convergencia(historial: List[float], ventana: int=50) -> Dict[str, float]:
    if len(historial) < 2:
        return {'convergencia': 0.0, 'tendencia': 0.0, 'varianza': 0.0}
    reciente = np.array(historial[-ventana:])
    var = float(np.var(reciente))
    if len(reciente) >= 10:
        pendiente = float(np.polyfit(range(len(reciente)), reciente, 1)[0])
    else:
        pendiente = 0.0
...
class _RingBufferNumpy:
    ...
    def to_array(self) -> np.ndarray:
        return np.array([self._data[(self._pos - self._count + i) % self.cap] for i in range(self._count)], dtype=self._dtype)
    ...
    def clear(self) -> None:
        self._pos = 0
        self._count = 0
        self._data = np.zeros((self.cap,), dtype=object)
...
class PerformanceProfiler:
    ...
    def stop(self, label: str) -> float:
        if not self._enabled or label not in self._start_times:
            return 0.0
    ...
    def get_all_avg(self) -> Dict[str, float]:
        return {k: self.get_avg(k) for k in self._timings}
...
class OptimizerFactory:
    ...
    def __init__(self):
        self._optimizers: Dict[str, Callable] = {'sgd': self._sgd, 'adam': self._adam, 'rmsprop': self._rmsprop, 'adagrad': self._adagrad}
    ...
    def _sgd(params: List[np.ndarray], grads: List[np.ndarray], lr: float, momentum: float=0.9) -> List[np.ndarray]:
        ...
        for i, (p, g) in enumerate(zip(params, grads)):
            v[i] = momentum * v[i] + g
            updated.append(p - lr * v[i])
        return updated

    @staticmethod
    def _adam(params: List[np.ndarray], grads: List[np.ndarray], lr: float, beta1: float=0.9, beta2: float=0.999, eps: float=1e-08) -> List[np.ndarray]:
        ...
        for i, (p, g) in enumerate(zip(params, grads)):
            ...
            mh = m[i] / (1 - beta1 ** t)
            vh = v[i] / (1 - beta2 ** t)
            updated.append(p - lr * mh / (np.sqrt(vh) + eps))
        return updated

    @staticmethod
    def _rmsprop(params: List[np.ndarray], grads: List[np.ndarray], lr: float, decay: float=0.9, eps: float=1e-08) -> List[np.ndarray]:
        cache = [np.zeros_like(p) for p in params]
        updated = []
        for i, (p, g) in enumerate(zip(params, grads)):
            cache[i] = decay * cache[i] + (1 - decay) * g * g
            updated.append(p - lr * g / (np.sqrt(cache[i]) + eps))
        return updated

    @staticmethod
    def _adagrad(params: List[np.ndarray], grads: List[np.ndarray], lr: float, eps: float=1e-08) -> List[np.ndarray]:
        cache = [np.zeros_like(p) for p in params]
        updated = []
        for i, (p, g) in enumerate(zip(params, grads)):
            cache[i] += g * g
...
class WeightStabilityTracker:
    ...
    def record(self, weights: np.ndarray) -> None:
        self._norms.append(float(np.linalg.norm(weights)))

    def get_stability(self) -> float:
        if len(self._norms) < 2:
            return 1.0
        diffs = np.diff(list(self._norms))
    ...
    def is_diverging(self, threshold: float=10.0) -> bool:
        if len(self._norms) < 10:
            return False
        recent = list(self._norms)[-10:]
        return max(recent) / max(1e-08, min(recent)) > threshold
...
class BatchPreprocessor:
    ...
    def update(self, batch: np.ndarray) -> None:
        ...
        self._running_mean = (1 - alpha) * self._running_mean + alpha * bs
        self._running_var = (1 - alpha) * self._running_var + alpha * bv

    def normalize(self, batch: np.ndarray) -> np.ndarray:
        return (batch - self._running_mean) / max(1e-08, np.sqrt(self._running_var))

    def denormalize(self, batch: np.ndarray) -> np.ndarray:
        return batch * max(1e-08, np.sqrt(self._running_var)) + self._running_mean
...
class TensorAccelerator:
    ...
    @staticmethod
    def batch_dot(vectors: np.ndarray, matrix: np.ndarray) -> np.ndarray:
    ...
    @staticmethod
    def l2_regularize(weights: np.ndarray, lambda_: float=0.001) -> np.ndarray:
        return lambda_ * weights

    @staticmethod
    def l1_regularize(weights: np.ndarray, lambda_: float=0.001) -> np.ndarray:
...
class WeightScheduler:
    ...
    def step(self) -> float:
        self._step += 1
        t = self._step / self._total
        if self._strategy == 'cosine':
            self._lr = 0.5 * self._lr * (1 + math.cos(math.pi * t))
        elif self._strategy == 'linear':
            self._lr = self._lr * (1 - t)
        elif self._strategy == 'step':
            self._lr = self._lr * 0.5 ** (self._step // 100)
        return self._lr
    ...
    def reset(self) -> None:
        self._step = 0
        self._lr = None if not hasattr(self, '_lr') else self._lr

class ConvergenceEngine:

    def __init__(self, patience: int=20, min_delta: float=0.0001, window: int=50):
    ...
    def update(self, value: float) -> bool:
        ...
        improved = value < self._best - self._delta
        if improved:
            self._best = value
            self._counter = 0
            return False
    ...
    def get_improvement_rate(self) -> float:
        ...
        return float(abs(d) / max(1e-08, len(self._history)))
...
class MemoryEfficientBuffer:
    ...
    def clear(self) -> None:
        self.pos = 0
        self.count = 0
        self.data = np.zeros((self.cap,), dtype=object)
...
class SerializationHelper:
    ...
    def serialize_neuron(neurona) -> bytes:
        ...
        if neurona.pesos is not None:
            state['pesos'] = neurona.pesos.tobytes()
        if neurona.sesgo is not None:
            state['sesgo'] = neurona.sesgo.tobytes()
        return pickle.dumps(state)

    @staticmethod
    def deserialize_neuron(data: bytes, neurona_class):
        state = pickle.loads(data)
        return neurona_class(state['input_size'], state['output_size'])

    @staticmethod
    def save_to_file(neurona, filepath: str) -> None:
        with open(filepath, 'wb') as f:
            f.write(SerializationHelper.serialize_neuron(neurona))

    @staticmethod
    def load_from_file(filepath: str, neurona_class):
        with open(filepath, 'rb') as f:
            return SerializationHelper.deserialize_neuron(f.read(), neurona_class)
...
__all__ = ['NeuronaRefuerzoBase', 'NeuronaRefuerzoQLearning', 'NeuronaRefuerzoPolicyGradient', 'NeuronaRefuerzoActorCritic', 'NeuronaRefuerzoDQN', 'NeuronaRefuerzoA3C', 'NeuronaRefuerzoPPO', 'NeuronaRefuerzoSAC', 'NeuronaRefuerzoTD3', 'NeuronaRefuerzoRainbowDQN', 'NeuronaRefuerzoIMPALA', 'LUCIA_RL_CONFIG', 'inicializar_pesos_he', 'inicializar_pesos_xavier', 'inicializar_pesos_lecun', 'inicializar_pesos_ortogonal', 'inicializar_pesos_espectral', 'inicializar_pesos_eficiente', 'calcular_recompensa_descontada', 'normalizar_recompensas', 'calcular_gae', 'aplicar_clip_gradientes', 'crear_buffer_experiencia', 'muestrear_buffer', 'calcular_entropy', 'calcular_loss', 'compute_returns', 'discount_cumulative_returns', 'huber_loss', 'mean_std_normalize', 'sample_gumbel_softmax', 'layer_init', 'count_parameters', 'get_optimizer', 'lr_scheduler', 'early_stopping_check', 'neuron_info', 'train_batch_neuron', 'evaluate_neurona', 'analizar_convergencia', 'comparar_inicializaciones', 'train_epoch', 'parallel_train', 'crear_buffer_priorizado', 'muestrear_priorizado', 'actualizar_prioridades', '_Ring', '_RingBufferNumpy', 'GradientAccumulator', 'LRUSymbolCache', 'PerformanceProfiler', 'OptimizerFactory', 'WeightStabilityTracker', 'BatchPreprocessor', 'TensorAccelerator', 'WeightScheduler', 'ConvergenceEngine', 'MemoryEfficientBuffer', 'SerializationHelper']

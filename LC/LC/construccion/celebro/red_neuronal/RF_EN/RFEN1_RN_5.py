...
class NeuronaRefuerzoA3C(NeuronaRefuerzoBase):
    ...
    def __init__(self, input_size: int=10, output_size: int=4, nombre: str='NeuronaRefuerzoA3C', learning_rate_actor: float=0.001, learning_rate_critic: float=0.002, gamma: float=0.99, lambda_gae: float=0.95, num_workers: int=4, update_frequency: int=20, usar_ortogonal: bool=True, entropy_coef: float=0.01, grad_clip: float=5.0, epsilon: float=1.0, epsilon_min: float=0.01, epsilon_decay: float=0.995, semilla: int=42, capacidad_historial: int=512):
        ...
        self.tiempo_upd = 0.0
        self.pasos = 0
        self.estadisticas_a3c = {'convergencia_actor_global': 0.0, 'convergencia_critic_global': 0.0, 'estabilidad_actor_global': 0.0, 'estabilidad_critic_global': 0.0, 'sincronizaciones': 0, 'updates_globales': 0, 'advantage_media': 0.0, 'advantage_std': 0.0, 'entropia_media': 0.0, 'workers_activos': 0, 'loss_media': 0.0, 'q_max': 0.0, 'q_min': 0.0, 'q_media': 0.0, 'q_std': 0.0, 'exploracion_rate': 0, 'exploitacion_rate': 0, 't_medio_fwd_us': 0.0, 't_medio_upd_us': 0.0, 'buffer_len': 0, 'valor_error_medio': 0.0, 'policy_loss_medio': 0.0}
    ...
    def _prep(self, estado: np.ndarray) -> np.ndarray:
        ...
        if x.shape[1] != self.input_size:
            raise ValueError(f'dim {x.shape[1]} != input {self.input_size}')
    ...
    def seleccionar_accion(self, estado, worker_id: int=0):
        ...
        if self.epsilon > self.rng.random():
            accion = int(self.rng.integers(0, self.output_size))
            self.estadisticas_a3c['exploracion_rate'] += 1
        else:
            accion = int(np.random.choice(self.output_size, p=prob[0]))
            self.estadisticas_a3c['exploitacion_rate'] += 1
    ...
    def calcular_gae_advantages(self, recompensas, valores):
        ...
        self.estadisticas_a3c['advantage_media'] = float(np.mean(advantages))
        self.estadisticas_a3c['advantage_std'] = float(np.std(advantages))
        return adv.tolist()

    def calcular_gradientes_worker(self, worker_id, estados, acciones, advantages, valores_objetivo):
        gap = np.zeros_like(self.pesos_actor_global)
        gas = np.zeros_like(self.sesgo_actor_global)
        gcp = np.zeros_like(self.pesos_critic_global)
        gcs = np.zeros_like(self.sesgo_critic_global)
        al = cl = 0.0
        n = max(1, len(estados))
        for s, a, adv, vo in zip(estados, acciones, advantages, valores_objetivo):
            prob = self.forward_actor(s, worker_id)
            v = self.forward_critic(s, worker_id)
            glp = np.zeros(self.output_size)
            glp[a] = 1.0 / (prob[0, a] + 1e-08)
            gap += np.outer(s[0], glp) * adv
            gas += glp * adv
            al += -np.log(prob[0, a] + 1e-08) * adv
            err = vo - v[0, 0]
            gcp += np.outer(s[0], np.array([err]))
            gcs += np.array([[err]])
            cl += 0.5 * err ** 2
        gap /= n
        gas /= n
        ...
        entropia = float(-np.mean([np.log(self.forward_actor(s, worker_id)[0, aa] + 1e-08) for s, aa in zip(estados, acciones)]))
        pl = float(-np.mean([np.log(self.forward_actor(s, worker_id)[0, aa] + 1e-08) * adv for s, aa, adv in zip(estados, acciones, advantages)]))
    ...
    def actualizar_pesos_worker(self, worker_id, gap, gas, gcp, gcs):
        with self.pesos_lock:
            t0 = time.perf_counter()
            lr_a = self.learning_rate_actor
            lr_c = self.learning_rate_critic
            ...
            self.estadisticas_a3c['t_medio_upd_us'] = 0.95 * self.estadisticas_a3c.get('t_medio_upd_us', 0) + 0.05 * (time.perf_counter() - t0) * 1000000.0

    def sincronizar_pesos_globales(self):
        with self.pesos_lock:
            t0 = time.perf_counter()
            pa = np.zeros_like(self.pesos_actor_global)
            sa = np.zeros_like(self.sesgo_actor_global)
            pc = np.zeros_like(self.pesos_critic_global)
            sc = np.zeros_like(self.sesgo_critic_global)
            for wid in range(self.num_workers):
                pa += self.pesos_actor_locales[wid]
                sa += self.sesgo_actor_locales[wid]
                pc += self.pesos_critic_locales[wid]
                sc += self.sesgo_critic_locales[wid]
            pa /= self.num_workers
            sa /= self.num_workers
            pc /= self.num_workers
            sc /= self.num_workers
            self.pesos_actor_global = pa
            self.sesgo_actor_global = sa
            self.pesos_critic_global = pc
            self.sesgo_critic_global = sc
            for wid in range(self.num_workers):
                self.pesos_actor_locales[wid] = pa.copy()
                self.sesgo_actor_locales[wid] = sa.copy()
                self.pesos_critic_locales[wid] = pc.copy()
                self.sesgo_critic_locales[wid] = sc.copy()
            self.estadisticas_a3c['sincronizaciones'] += 1
            self.estadisticas_a3c['updates_globales'] += 1
            self.historial_sincronizaciones.append(time.time())
            self.estadisticas_a3c['t_medio_fwd_us'] = 0.95 * self.estadisticas_a3c.get('t_medio_fwd_us', 0) + 0.05 * (time.perf_counter() - t0) * 1000000.0
    ...
    def entrenar_paso(self, s, a, r, ns, d: bool=False):
        ...
        self.pasos += 1
        self.estadisticas_a3c['buffer_len'] = len(self.buffer)
    ...
    def _convergencia_rapida(self):
        if self.pesos_actor_global is not None:
            pm = float(np.mean(np.abs(self.pesos_actor_global)))
            self._delta_ema = 0.02 * pm + 0.98 * self._delta_ema
    ...
    def calcular_loss(self, estados, acciones, ventajas, valores_objetivo):
        ...
        for s, a, adv, vo in zip(estados, acciones, ventajas, valores_objetivo):
            prob = self.forward_actor(s, 0)
            val = self.forward_critic(s, 0)
            pl += -np.log(prob[0, a] + 1e-08) * adv
            cl += 0.5 * (vo - val[0, 0]) ** 2
        return {'policy_loss': float(pl / n), 'value_loss': float(cl / n), 'total_loss': float((pl + cl) / n)}

    def limpiar_historiales(self):
        self.historial_qvalues.clear()
    ...
    def actualizar_buffer(self, experiencia):
        ...
        if len(self.buffer) > 10240:
            del self.buffer[0:len(self.buffer) - 10240]
    ...
    def _calcular_convergencia_ac(self):
        hl = self.historial_actor_loss
        hlc = self.historial_critic_loss
        if len(hl) < 10 or len(hlc) < 10:
            return
    ...
    def calcular_convergencia_global(self):
        ...
        if hasattr(self, 'historial_pesos_actor_global'):
            pa = np.array(self.historial_pesos_actor_global[-5:])
            vp = np.mean(np.var(pa, axis=0))
            c = 1.0 / (1.0 + vp)
            self.estadisticas_a3c['convergencia_actor_global'] = c
            self.estadisticas_a3c['convergencia_critic_global'] = c
    ...
    def calcular_estabilidad_global(self):
        ...
        self.estadisticas_a3c['estabilidad_actor_global'] = ea
        self.estadisticas_a3c['estabilidad_critic_global'] = ec
        return (ea, ec)
    ...
    def obtener_estadisticas_a3c(self):
        ...
        self.estadisticas_a3c['loss_media'] = self._loss_ema
        self.estadisticas_a3c['buffer_len'] = len(self.buffer)
        return {'learning_rate_actor': self.learning_rate_actor, 'learning_rate_critic': self.learning_rate_critic, 'gamma': self.gamma, 'lambda_gae': self.lambda_gae, 'num_workers': self.num_workers, 'update_frequency': self.update_frequency, 'convergencia_actor_global': self.estadisticas_a3c['convergencia_actor_global'], 'convergencia_critic_global': self.estadisticas_a3c['convergencia_critic_global'], 'estabilidad_actor_global': ea, 'estabilidad_critic_global': ec, 'sincronizaciones': self.estadisticas_a3c['sincronizaciones'], 'updates_globales': self.estadisticas_a3c['updates_globales'], 'entropy_coef': self.entropy_coef, 'epsilon': self.epsilon, 'loss_media': self.estadisticas_a3c.get('loss_media', 0.0), 'q_max': self.estadisticas_a3c['q_max'], 'q_min': self.estadisticas_a3c['q_min'], 'q_media': self.estadisticas_a3c['q_media'], 'q_std': self.estadisticas_a3c['q_std'], 't_medio_fwd_us': self.estadisticas_a3c.get('t_medio_fwd_us', 0), 't_medio_upd_us': self.estadisticas_a3c.get('t_medio_upd_us', 0), 'policy_loss_medio': self.estadisticas_a3c.get('policy_loss_medio', 0.0), 'valor_error_medio': self.estadisticas_a3c.get('valor_error_medio', 0.0)}

    def verificar_estabilidad(self):
        est = {}
        ca = self.estadisticas_a3c['convergencia_actor_global']
        cc = self.estadisticas_a3c['convergencia_critic_global']
        est['actor_convergencia_ok'] = ca > 0.7
        est['critic_convergencia_ok'] = cc > 0.7
        ea, ec = self.calcular_estabilidad_global()
        est['actor_estable'] = ea > 0.7
        est['critic_estable'] = ec > 0.7
    ...
    def reinicializar_con_parametros(self, learning_rate_actor=None, learning_rate_critic=None, gamma=None, lambda_gae=None, num_workers=None):
    ...
    def a_dict(self) -> Dict[str, Any]:
        return {'input_size': self.input_size, 'output_size': self.output_size, 'nombre': self.nombre, 'pesos_actor': self.pesos_actor_global.tolist() if self.pesos_actor_global is not None else None, 'pesos_critic': self.pesos_critic_global.tolist() if self.pesos_critic_global is not None else None, 'sesgo_actor': self.sesgo_actor_global.tolist() if self.sesgo_actor_global is not None else None, 'sesgo_critic': self.sesgo_critic_global.tolist() if self.sesgo_critic_global is not None else None}
    ...
    def cargar(self, ruta: str) -> None:
        ...
        if d.get('pesos_actor') is not None:
            self.pesos_actor_global = np.asarray(d['pesos_actor'], dtype=_dt())
        if d.get('pesos_critic') is not None:
            self.pesos_critic_global = np.asarray(d['pesos_critic'], dtype=_dt())
        if d.get('sesgo_actor') is not None:
            self.sesgo_actor_global = np.asarray(d['sesgo_actor'], dtype=_dt())
        if d.get('sesgo_critic') is not None:
            self.sesgo_critic_global = np.asarray(d['sesgo_critic'], dtype=_dt())
        for wid in range(self.num_workers):
            self.pesos_actor_locales[wid] = self.pesos_actor_global.copy()
            self.sesgo_actor_locales[wid] = self.sesgo_actor_global.copy()
            self.pesos_critic_locales[wid] = self.pesos_critic_global.copy()
    ...
    def __str__(self):
        return f'NeuronaRefuerzoA3C(entrada={self.input_size},salida={self.output_size},lr_a={self.learning_rate_actor},lr_c={self.learning_rate_critic},workers={self.num_workers})'
...
def crear_neurona_a3c(input_size, output_size, configuracion=None):
    c = configuracion or {}
    return NeuronaRefuerzoA3C(input_size=input_size, output_size=output_size, nombre=c.get('nombre', 'NeuronaRefuerzoA3C'), learning_rate_actor=c.get('learning_rate_actor', 0.001), learning_rate_critic=c.get('learning_rate_critic', 0.002), gamma=c.get('gamma', 0.99), lambda_gae=c.get('lambda_gae', 0.95), num_workers=c.get('num_workers', 4), update_frequency=c.get('update_frequency', 20), usar_ortogonal=c.get('usar_ortogonal', True), entropy_coef=c.get('entropy_coef', 0.01))

def analizar_a3c(neurona):
    return {'estadisticas': neurona.obtener_estadisticas_a3c(), 'estable': neurona.verificar_estabilidad()}
RFEN5_CONFIG = {'inicializacion_preferida': 'ortogonal', 'learning_rate_actor_default': 0.001, 'learning_rate_critic_default': 0.002, 'gamma_default': 0.99, 'lambda_gae_default': 0.95, 'num_workers_default': 4, 'update_frequency_default': 20, 'entropy_coef_default': 0.01, 'umbral_convergencia': 0.7, 'umbral_estabilidad': 0.7}

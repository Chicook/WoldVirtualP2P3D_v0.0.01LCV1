...
_MODULE_MAP: Dict[str, Tuple[str, str, str, str]] = {'adamw': ('RN1', 'AdamWOptimizer', 'create_adamw_optimizer', 'quick_adamw'), 'radam': ('RN2', 'RAdamOptimizer', 'create_radam_optimizer', 'quick_radam'), 'lookahead': ('RN3', 'LookaheadOptimizer', 'create_lookahead_optimizer', 'quick_lookahead'), 'nadam': ('RN4', 'NadamOptimizer', 'create_nadam_optimizer', 'quick_nadam'), 'lamb': ('RN5', 'LAMBOptimizer', 'create_lamb_optimizer', 'quick_lamb'), 'adabelief': ('RN6', 'AdaBeliefOptimizer', 'create_adabelief_optimizer', 'quick_adabelief'), 'lion': ('RN7', 'LionOptimizer', 'create_lion_optimizer', 'quick_lion'), 'sam': ('RN8', 'SAMOptimizer', 'create_sam_optimizer', 'quick_sam'), 'swats': ('RN9', 'SWATSOptimizer', 'create_swats_optimizer', 'quick_swats'), 'integrated': ('RN10', 'IntegratedNeuralWeightOptimizer', 'create_integrated_neural_weight_optimizer', 'quick_integrated')}
_ALIASES: Dict[str, str] = {'rn1': 'adamw', 'rn2': 'radam', 'rn3': 'lookahead', 'rn4': 'nadam', 'rn5': 'lamb', 'rn6': 'adabelief', 'rn7': 'lion', 'rn8': 'sam', 'rn9': 'swats', 'rn10': 'integrated'}
    ...
    def _load(self) -> Any:
        if self._module is None:
            try:
                ...
                raise ImportError(f'RNP: fallo al importar {self._module_name}: {exc}') from exc
        return self._module
...
class RNPRegistry:
    ...
    def _get_lazy(cls, key: str) -> _LazyModule:
        ...
        if key not in _MODULE_MAP:
            ...
            raise KeyError(f'RNPRegistry: neurona {key!r} no encontrada. Disponibles: {available}')
    ...
    def list_all(cls) -> Dict[str, Dict[str, str]]:
        ...
        for key, (mod, cls_name, factory_fn, quick_fn) in _MODULE_MAP.items():
            result[key] = {'module': mod, 'class': cls_name, 'factory_fn': factory_fn, 'quick_fn': quick_fn}
        return result
...
class RNPBenchmark:
    ...
    @staticmethod
    def run_single(key: str, model: Optional[Any]=None, config: Optional[Any]=None) -> Dict[str, Any]:
        ...
        entry: Dict[str, Any] = {'key': key, 'success': False, 'elapsed_s': 0.0, 'error': None, 'result': None}
        t0 = time.perf_counter()
        try:
            result = RNPFactory.quick_run(key, model=model, config=config)
            entry['success'] = result.get('success', False) if isinstance(result, dict) else bool(result)
            entry['result'] = result
        except Exception as exc:
    ...
    @staticmethod
    def run_all(model: Optional[Any]=None, keys: Optional[List[str]]=None, config: Optional[Any]=None, verbose: bool=False) -> Dict[str, Any]:
        ...
        summary: Dict[str, Any] = {'total': len(targets), 'successful': len(successful), 'failed': len(failed), 'fastest': rankings[0] if rankings else None, 'rankings': rankings}
        if verbose:
            sep = '=' * 60
            print(f'\n{sep}')
            ...
            print(f"\n  OK: {len(successful)}/{len(targets)} | Mas rapido: {summary['fastest']}")
            print(f'{sep}\n')
        return {'summary': summary, 'results': results, 'rankings': rankings}

    @staticmethod
    def compare(keys: List[str], model: Optional[Any]=None, config: Optional[Any]=None) -> Dict[str, Any]:
...
def describe() -> str:
    ...
    lines = [f'RNP v{__version__} (API {__api_level__}) - {__author__}', f'Algoritmos disponibles ({len(_MODULE_MAP)}):']
...
def health_check(verbose: bool=False) -> Dict[str, Any]:
    ...
    report: Dict[str, Any] = {'status': status, 'version': __version__, 'api_level': __api_level__, 'available': ok_keys, 'unavailable': bad_keys, 'details': details}
    if verbose:
...
__all__ = ['__version__', '__api_level__', '__author__', 'RNPRegistry', 'RNPFactory', 'RNPBenchmark', 'get_version', 'get_api_level', 'list_optimizers', 'probe_all', 'describe', 'health_check', 'RN1', 'RN2', 'RN3', 'RN4', 'RN5', 'RN6', 'RN7', 'RN8', 'RN9', 'RN10']
logger.info('RNP v%s cargado - %d neuronas disponibles (API %d)', __version__, len(_MODULE_MAP), __api_level__)

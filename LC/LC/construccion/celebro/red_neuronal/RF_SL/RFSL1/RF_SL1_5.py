...
class DecisionTreeOptimizer(NeuronaMemoriaBase):
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray, depth: int) -> Dict:
        ...
        if gain < 1e-08:
            return {'leaf': True, 'value': float(np.bincount(y.astype(int)).argmax())}
    ...
    def prune(self, X: np.ndarray, y: np.ndarray, validation_fraction: float=0.2) -> None:
        ...
        if self._tree is not None:
            self._tree = self._prune_tree(self._tree, X[idx], y[idx])

    def _prune_tree(self, node: Dict, X: np.ndarray, y: np.ndarray) -> Dict:
        if node.get('leaf'):
            return node
        if len(X) < self._min_samples_split:
            return {'leaf': True, 'value': float(np.bincount(y.astype(int)).argmax())}
    ...
    def save(self, filepath: str) -> None:
        state = {'pesos': self.pesos, 'sesgo': self.sesgo, 'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'max_depth': self._max_depth, 'min_samples_split': self._min_samples_split, 'criterion': self._criterion, 'is_fitted': self._is_fitted, 'tree': self._tree, 'feature_importances': self._feature_importances, 'convergence_history': self._convergence_history}
    ...
    def get_config(self) -> Dict[str, Any]:
        return {'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'max_depth': self._max_depth, 'criterion': self._criterion}

    def summary(self) -> str:
        cfg = self.get_config()
        stats = self.obtener_estadisticas()
        return '\n'.join([f"=== {cfg['nombre']} ===", f"Depth: {cfg['max_depth']}, Criterion: {cfg['criterion']}", f"Input: {cfg['input_size']}, Output: {cfg['output_size']}", f"Fitted: {stats.get('is_fitted', False)}, Top feature: {stats.get('top_feature', 'N/A')}", f'Params: {self.params_count()}'])
...
class TreeSplitter:
    ...
    def find_best_split(self, X: np.ndarray, y: np.ndarray) -> Tuple[int, float, float]:
        if self.criterion == 'gini':
            score_fn = self._gini_score
        elif self.criterion == 'entropy':
            score_fn = self._entropy_score
        else:
            score_fn = self._mse_score
        best_f, best_t, best_g = (0, 0.0, 0.0)
        parent = score_fn(y)
        for f in range(X.shape[1]):
            for t in np.unique(X[:, f]):
    ...
    @staticmethod
    def _mse_score(y: np.ndarray) -> float:
        return float(np.var(y))
...
class FeatureImportance:
    ...
    def add(self, features: np.ndarray, gains: np.ndarray) -> None:
        for f, g in zip(features, gains):
            self._importances[int(f)] += float(g)

    def get(self) -> np.ndarray:
        return self._importances.copy()

    def top_k(self, k: int=3) -> np.ndarray:
        return np.argsort(self._importances)[-k:][::-1]

    def normalized(self) -> np.ndarray:
        total = self._importances.sum()
        return self._importances / max(1e-08, total)
...
class CriterionSelector:
    ...
    def list_criteria() -> List[str]:
        return ['gini', 'entropy', 'mse']

class TreeEnsemble:

    def __init__(self, n_trees: int=10):
        self.n_trees = n_trees
        self._trees: List[DecisionTreeOptimizer] = []

    def add_tree(self, tree: DecisionTreeOptimizer) -> None:

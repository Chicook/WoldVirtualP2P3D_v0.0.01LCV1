...
class SupportVectorMachineOptimizer(NeuronaMemoriaBase):
    ...
    def kernel(self, X1: np.ndarray, X2: np.ndarray) -> np.ndarray:
        if self._kernel_type == 'linear':
            return X1 @ X2.T
        if self._kernel_type == 'poly':
            return (X1 @ X2.T + 1) ** self._degree
        if self._kernel_type == 'sigmoid':
            return np.tanh(self._gamma * (X1 @ X2.T) + self._degree)
    ...
    def compute_margin(self) -> float:
        if self.pesos is not None:
            return float(1.0 / (np.linalg.norm(self.pesos) + 1e-08))
        return 0.0
    ...
    def save(self, filepath: str) -> None:
        state = {'pesos': self.pesos, 'sesgo': self.sesgo, 'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'kernel_type': self._kernel_type, 'C': self._C, 'gamma': self._gamma, 'degree': self._degree, 'bias': self._bias, 'is_fitted': self._is_fitted, 'support_vectors': self._support_vectors, 'support_labels': self._support_labels, 'alphas': self._alphas, 'pasos': self.pasos, 'convergence_history': self._convergence_history}
    ...
    def get_config(self) -> Dict[str, Any]:
        return {'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'kernel_type': self._kernel_type, 'C': self._C, 'gamma': self._gamma, 'degree': self._degree, 'max_iter': self._max_iter}

    def summary(self) -> str:
        cfg = self.get_config()
        stats = self.obtener_estadisticas()
        return '\n'.join([f"=== {cfg['nombre']} ===", f"Kernel: {cfg['kernel_type']}, C: {cfg['C']}, gamma: {cfg['gamma']}", f"Input: {cfg['input_size']}, Output: {cfg['output_size']}", f"Fitted: {stats.get('is_fitted', False)}, Margin: {stats.get('margin', 0):.4f}", f"Support vectors: {stats.get('n_support_vectors', 0)}, Params: {self.params_count()}"])
...
class KernelEngine:
    ...
    def compute(self, X1: np.ndarray, X2: np.ndarray) -> np.ndarray:
        ...
        if self.kernel_type == 'sigmoid':
            return np.tanh(self.gamma * (X1 @ X2.T) + self.degree)
        sq1 = np.sum(X1 ** 2, axis=1).reshape(-1, 1)
        sq2 = np.sum(X2 ** 2, axis=1).reshape(1, -1)
        dist = sq1 + sq2 - 2 * (X1 @ X2.T)
...
class SMOTrainer:
    ...
    def __init__(self, C: float=1.0, tol: float=0.001, max_iter: int=1000):
        self.C = C
        self.tol = tol
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(n):
                ...
                if y[i] * Ei < -self.tol and alphas[i] < self.C or (y[i] * Ei > self.tol and alphas[i] > 0):
                    ...
                    if abs(alphas[j] - alpha_j_old) < 1e-05:
                        continue
                    alphas[i] += y[i] * y[j] * (alpha_j_old - alphas[j])
            if np.max(np.abs(alphas - alpha_prev)) < self.tol:
                break
        sv = alphas > 1e-05
    ...
    def get_objective_history(self) -> List[float]:
        return self._objective_history.copy()

    def is_converged(self, tol: float=0.0001) -> bool:
...
class SupportVectorIdentifier:
    ...
    def __init__(self, threshold: float=1e-05):
    ...
    def count(self) -> int:
        return len(self._sv_indices) if self._sv_indices is not None else 0
...
class SVMClassifier:
    ...
    def train(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        return {'n_support_vectors': self.identifier.count(), 'margin': margin, 'training_time': self._training_time, 'kernel': self.kernel_type, 'C': self.C, 'converged': True}

    def predict(self, X: np.ndarray) -> np.ndarray:
        if not self._is_trained:
            raise RuntimeError('SVM no entrenado')

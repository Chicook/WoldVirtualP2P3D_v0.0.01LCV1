...
class LinearRegressionOptimizer(NeuronaMemoriaBase):
    ...
    def r2_score(self, y_true: np.ndarray, y_pred: np.ndarray) -> float:
        ...
        score = 1.0 - ss_res / max(1e-08, ss_tot)
    ...
    def evaluate(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        preds = self.predict(X)
        return {'predictions': preds, 'r2': self.r2_score(y, preds), 'mae': self.mae(y, preds), 'rmse': self.rmse(y, preds), 'n_samples': len(X)}
...
class NormalEquation:
    ...
    def ridge_solve(X: np.ndarray, y: np.ndarray, alpha: float=0.01) -> np.ndarray:
        ...
        reg_matrix[-1, -1] = 0.0
        theta = np.linalg.solve(np.dot(X_bias.T, X_bias) + reg_matrix, np.dot(X_bias.T, y))
        return theta
...
class FeatureStandardizer:
    ...
    def get_params(self) -> Dict[str, np.ndarray]:
        return {'mean': self._mean.copy() if self._mean is not None else None, 'std': self._std.copy() if self._std is not None else None}
...
class ModelMetrics:
    ...
    def evaluate_all(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        return {'r2': ModelMetrics.r2(y_true, y_pred), 'mae': ModelMetrics.mae(y_true, y_pred), 'rmse': ModelMetrics.rmse(y_true, y_pred), 'mape': ModelMetrics.mape(y_true, y_pred)}

...
class KNearestNeighborsOptimizer(NeuronaMemoriaBase):
    ...
    def _cosine(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        ...
        return 1.0 - dot / np.maximum(1e-08, norm_a * norm_b)

    def _compute_distances(self, X: np.ndarray) -> np.ndarray:
        if self._distance_metric == 'manhattan':
            return self._manhattan(X, self._data)
        if self._distance_metric == 'cosine':
            return self._cosine(X, self._data)
    ...
    def save(self, filepath: str) -> None:
        state = {'pesos': self.pesos, 'sesgo': self.sesgo, 'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'k': self._k, 'distance_metric': self._distance_metric, 'is_fitted': self._is_fitted, 'pasos': self.pasos, 'data': self._data, 'labels': self._labels, 'accuracy': self._accuracy, 'convergence_history': self._convergence_history}
    ...
    def get_config(self) -> Dict[str, Any]:
        return {'nombre': self.nombre, 'input_size': self.input_size, 'output_size': self.output_size, 'k': self._k, 'distance_metric': self._distance_metric}

    def summary(self) -> str:
        cfg = self.get_config()
        stats = self.obtener_estadisticas()
        return '\n'.join([f"=== {cfg['nombre']} ===", f"K: {cfg['k']}, Metric: {cfg['distance_metric']}", f"Input: {cfg['input_size']}, Output: {cfg['output_size']}", f"Fitted: {stats.get('is_fitted', False)}, Acc: {stats.get('accuracy', 0):.4f}", f"Samples: {stats.get('n_samples', 0)}, Params: {self.params_count()}"])
...
class FeatureNormalizer:
    ...
    def fit(self, X: np.ndarray) -> 'FeatureNormalizer':
        ...
        self._std = np.std(X, axis=0) + 1e-08
        return self

    def transform(self, X: np.ndarray) -> np.ndarray:
        if self._mean is None:
            raise RuntimeError('Normalizer no ajustado')
...
class KNNClassifier:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        return np.array([self.voter.aggregate(self._labels[np.argpartition(DistanceMetric.euclidean(X[i:i + 1], self._data)[0], self.k)[:self.k]]) for i in range(len(X))])
...
class ModelEvaluator:
    ...
    def evaluate_k_range(self, X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray) -> Dict[int, float]:

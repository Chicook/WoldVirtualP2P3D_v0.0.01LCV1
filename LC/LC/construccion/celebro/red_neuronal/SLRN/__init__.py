...
__license__: str = 'MIT'
__all__: List[str] = ['SupervisedLearningNeuralConfig', 'SupervisedLearningNeuralMetrics', 'SupervisedLearningNeuralResult', 'BaseSupervisedLearningNeuralOptimizer', 'SLRNSystem', 'SupervisedLearningNeuralSystem', 'create_integrated_supervised_learning_neural_optimizer', 'OPTIMIZER_REGISTRY']
logger = logging.getLogger(__name__)
OPTIMIZER_REGISTRY: Dict[str, str] = {'backprop': 'SL1', 'sgd': 'SL2', 'rmsprop': 'SL3', 'adagrad': 'SL4', 'adadelta': 'SL5', 'adamw': 'SL6', 'adamax': 'SL7', 'amsgrad': 'SL8', 'adabound': 'SL9', 'integrated': 'SL10'}
SLRN_META: Dict[str, str] = {'version': __version__, 'description': 'Supervised Learning Neural Networks 2026 - 10 Optimizers + Meta-Ensemble', 'author': __author__, 'license': __license__, 'created': '2026', 'algorithms': ', '.join(OPTIMIZER_REGISTRY.keys())}
...
class SupervisedLearningNeuralMetrics:
    ...
    def summary(self) -> str:
        ...
        return f'[{self.algorithm_name}] Loss: {self.initial_loss:.4f} -> {self.final_loss:.4f} ({self.loss_reduction_pct:.1f}% red.) | Score: {self.overall_score:.4f} | Iter: {self.convergence_iterations} | {self.timestamp}'
...
class SLRNSystem:
    ...
    def _load_optimizer(self, key: str) -> BaseSupervisedLearningNeuralOptimizer:
        ...
        factory_map = {'backprop': 'create_backpropagation_optimizer', 'sgd': 'create_sgd_optimizer', 'rmsprop': 'create_rmsprop_optimizer', 'adagrad': 'create_adagrad_optimizer', 'adadelta': 'create_adadelta_optimizer', 'adamw': 'create_adam_optimizer', 'adamax': 'create_adamax_optimizer', 'amsgrad': 'create_amsgrad_optimizer', 'adabound': 'create_adabound_optimizer', 'integrated': 'create_integrated_supervised_learning_optimizer'}
    ...
    def run(self, key: str, model: Any, data_loader: Any=None, criterion: Any=None) -> SupervisedLearningNeuralResult:
    ...
    def run_all(self, model: Any, data_loader: Any=None, criterion: Any=None, keys: Optional[List[str]]=None) -> Dict[str, SupervisedLearningNeuralResult]:
        ...
        for key in targets:
            try:
                ...
                self._results[key] = SupervisedLearningNeuralResult(success=False, optimized_model=None, metrics=None, optimization_history=[], best_weights={}, theoretical_analysis={}, performance_analysis={}, recommendations=[], error_message=str(exc))
...
def create_integrated_supervised_learning_neural_optimizer(config: Optional[SupervisedLearningNeuralConfig]=None) -> SLRNSystem:
...
def list_optimizers() -> List[str]:
    ...
    return list(OPTIMIZER_REGISTRY.keys())
logger.info('SLRN v%s cargado | 10 optimizadores neuronales supervisados 2026 disponibles | keys=%s', __version__, list(OPTIMIZER_REGISTRY.keys()))

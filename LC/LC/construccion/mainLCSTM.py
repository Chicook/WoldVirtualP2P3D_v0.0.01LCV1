...
try:
    ...
    get_procesador_rplc = None
    reprocesar_con_metricas = None
try:
...
class OrquestadorSistemaLucIA:
    ...
    def inicializar_subsistemas(self) -> bool:
        ...
        try:
            ...
            print(f'  [1/4] Blockchain BKSVCB    : \x1b[38;5;48mACTIVA\x1b[0m | Bloques: \x1b[38;5;220m{len(self.servidor_bks.cadena)}\x1b[0m')
            self.servidor_bks.mostrar_cadena_hashes_terminal()
            iniciar_servidor_blockchain(puerto=8545)
        except Exception as e_bks:
            print(f'  [1/4] Blockchain BKSVCB    : \x1b[38;5;203mERROR ({e_bks})\x1b[0m')
            return False
        try:
            ...
            print(f'  [2/4] Conversor PSNRCV     : \x1b[38;5;48mOK\x1b[0m | Sinapsis: \x1b[38;5;141m{num_neu} Activas\x1b[0m')
        except Exception as e_psn:
            print(f'  [2/4] Conversor PSNRCV     : \x1b[38;5;203mERROR ({e_psn})\x1b[0m')
            return False
        try:
            res_t = self.servidor_bks.transformar_ledger_a_pesos_neuronales()
            self.servidor_bks.arrancar_actualizador_red()
            print(f"  [3/4] Ledger -> Pesos PSNRL: \x1b[38;5;48m{res_t.get('total_bloques', 0)} bloques\x1b[0m | Norma: \x1b[38;5;214m{res_t.get('norma_acumulada', 0.0)}\x1b[0m")
        except Exception as e_led:
            print(f'  [3/4] Ledger -> Pesos      : \x1b[38;5;214mAVISO ({e_led})\x1b[0m')
        try:
            from LC.celebro.CMFG.SBSTM import obtener_clase_sesion
            cls_sesion = obtener_clase_sesion()
            self.sesion_p2p = cls_sesion()
            print(f'  [4/4] Sesion SNSBSTNPRB    : \x1b[38;5;48mACOPLADA\x1b[0m | Motor STYLOS: \x1b[38;5;51mACTIVO\x1b[0m\n')
        except Exception as e_sbs:
            print(f'  [4/4] Sesion SNSBSTNPRB    : \x1b[38;5;214mAVISO ({e_sbs})\x1b[0m\n')
        self.activa = True
        return True
    ...
    def procesar_turno_dialogo(self, prompt: str) -> None:
        ...
        try:
            ...
            print(f'\x1b[38;5;203m  [Blockchain Error] {ex_bks}\x1b[0m')
    ...
    def _generar_reflejo_interno(self, prompt: str, info: Dict[str, Any]) -> str:
        ...
        return f'Estimulo cognitivo asimilado en red distribuida. Tono: **{tono}** | Valencia afectiva: `{val:+.3f}` | Delta sinaptico: `{norma:.5f}`.\nPesos neuronales sincronizados y activos en persistencia `PSNRL`.'
    ...
    def listar_modelos_gratuitos(self) -> None:
        ...
        print('\n\x1b[38;5;51m' + '=' * 68 + '\x1b[0m')
        print(f'  \x1b[1;37mCATALOGO DE MODELOS GRATUITOS IAFREE ({len(modelos)} disponibles):\x1b[0m')
        print('\x1b[38;5;51m' + '=' * 68 + '\x1b[0m')
        for m in modelos:
            pref = '\x1b[38;5;48m▶\x1b[0m ' if m['id'] == self.cliente_iafree.gestor.obtener_modelo_activo()['id'] else '  '
            print(f"{pref}\x1b[38;5;51m{m['id']:<42}\x1b[0m | {m['contexto']} tok | {m['nombre']}")
        print('\x1b[38;5;51m' + '-' * 68 + '\x1b[0m\n')
    ...
    def ejecutar_bucle_interactivo(self) -> None:
        ...
        while self.activa:
            try:
                entrada = input('\x1b[38;5;214mLucIA> Tu:\x1b[0m ').strip()
            ...
            if entrada.lower() in ('salir', 'exit', 'quit'):
                print('\n\x1b[38;5;214mIniciando protocolo de salida y sincronizacion...\x1b[0m')
                break
            ...
            if entrada.lower().startswith('modelo '):
                ...
                if self.cliente_iafree and self.cliente_iafree.gestor.seleccionar_por_id(nuevo_mod):
                    print(f'  \x1b[38;5;48mModelo gratuito seleccionado: {nuevo_mod}\x1b[0m')
                else:
                    print(f"  \x1b[38;5;214mModelo '{nuevo_mod}' no encontrado en catalogo :free.\x1b[0m")
                continue
    ...
    def cerrar_sistema(self) -> None:
        ...
        with self.lock:
            if not self.servidor_bks:
                return
            print('\n\x1b[38;5;51m' + '=' * 76 + '\x1b[0m')
            print('  \x1b[1;37mCONSOLIDANDO ESTADO COGNITIVO Y PERSISTENCIA FINAL...\x1b[0m')
            try:
                blk = self.servidor_bks.minar_transacciones_pendientes()
                if blk:
                    print(f'  Bloque final consolidado: \x1b[38;5;220m{blk.hash_bloque[:28]}...\x1b[0m')
            except Exception:
                pass
            try:
                self.servidor_bks.cerrar_sesion_y_subir_ipfs()
                print('  \x1b[38;5;48mSincronizacion IPFS de bloques y pesos completada exitosamente.\x1b[0m')
            except Exception as e_close:
                print(f'  \x1b[38;5;214mCierre IPFS: {e_close}\x1b[0m')
            print('\x1b[38;5;51m' + '=' * 76 + '\x1b[0m\n')
            self.servidor_bks = None
...
def main() -> int:
    ...
    try:
        ...
        ejecutar_flujo_sesion_local()
    except Exception as e_pre:
        print(f'[mainLCSTM] Pre-sesion IALOCAL omitida: {e_pre}')

...
class NeuronaEntradaNormalizada(NeuronaEntradaBase):
    ...
    def _normalizar(self, x: np.ndarray) -> np.ndarray:
        p = self.params_norm
        if p['media'] is None:
            p['media'] = np.mean(x, axis=0, keepdims=True)
            p['std'] = np.std(x, axis=0, keepdims=True)
            p['min'] = np.min(x, axis=0, keepdims=True)
            p['max'] = np.max(x, axis=0, keepdims=True)
        if self.metodo == 'z_score':
            return (x - p['media']) / (p['std'] + 1e-8)
        elif self.metodo == 'min_max':
            return (x - p['min']) / (p['max'] - p['min'] + 1e-8)
        return x

    def forward(self, entrada: np.ndarray) -> np.ndarray:
        if self.pesos is None:
            self.inicializar_pesos()
        entrada_norm = self._normalizar(entrada)
    ...
    def backward(self, gradiente_salida: np.ndarray, entrada: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        entrada_norm = self._normalizar(entrada)
        grad_pesos = np.dot(entrada_norm.T, gradiente_salida)
    ...
    def __str__(self) -> str:
        return f"{self.nombre}(ent={self.input_size},sal={self.output_size},norm={self.metodo})"

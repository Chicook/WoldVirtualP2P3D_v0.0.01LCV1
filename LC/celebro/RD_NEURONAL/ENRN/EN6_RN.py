...
class NeuronaEntradaDropout(NeuronaEntradaBase):
    ...
    def forward(self, entrada: np.ndarray) -> np.ndarray:
        ...
        if self.modo_entrenamiento and self.dropout_rate > 0:
            mascara = np.random.random(salida.shape) > self.dropout_rate
            salida = salida * mascara / (1.0 - self.dropout_rate)
    ...
    def obtener_estadisticas_completas(self) -> Dict[str, Any]:
        ...
        stats.update({'dropout_rate': self.dropout_rate, 'varianza_real': self.varianza_real,
    ...
    def verificar_estabilidad(self) -> Dict[str, bool]:
        ...
        if self.pesos is not None:
            ...
            est['dropout_activo'] = self.dropout_rate > 0
        return est

    def __str__(self) -> str:
        return f"{self.nombre}(ent={self.input_size},sal={self.output_size},drop={self.dropout_rate})"

...
class NeuronaEntradaReLU(NeuronaEntradaBase):
    ...
    def forward(self, entrada: np.ndarray) -> np.ndarray:
        ...
        salida = self._activar(suma)
        tasa = float(np.sum(salida > 0)) / salida.size
        self.tasa_activacion = 0.9 * self.tasa_activacion + 0.1 * tasa if self.historial_activaciones else tasa
    ...
    def backward(self, gradiente_salida: np.ndarray, entrada: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        if self.historial_activaciones:
            antes = self.historial_activaciones[-1]
            grad_act = self._gradiente_activacion(antes)
    ...
    def obtener_estadisticas_completas(self) -> Dict[str, Any]:
        ...
        stats.update({'tipo_relu': self.tipo_relu, 'tasa_activacion': self.tasa_activacion,
    ...
    def __str__(self) -> str:
        return f"{self.nombre}(ent={self.input_size},sal={self.output_size},relu={self.tipo_relu})"

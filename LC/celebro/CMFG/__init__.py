...
_SUBSYSTEM_REGISTRY: Dict[str, Dict[str, Any]] = {
    "PSNRCV": {
        "modulo": "LC.celebro.CMFG.PSNRCV",
        "descripcion": "Conversor y Transductor Neural de 50 Neuronas",
        "requerido": True,
        "cargado": False,
    },
    "pesos_vivos": {
        "modulo": "LC.celebro.CMFG.pesos_vivos",
        "descripcion": "Monitor Dinamico ANSI de Pesos Vivos",
        "requerido": True,
        "cargado": False,
    },
    "ipfs_manager": {
        "modulo": "LC.celebro.CMFG.ipfs_manager",
        "descripcion": "Persistencia Descentralizada IPFS y Limpieza Atomica",
        "requerido": True,
        "cargado": False,
    },
    "IPFSonl": {
        "modulo": "LC.celebro.CMFG.IPFSonl",
        "descripcion": "Orquestador y Daemon Kubo IPFS Local",
        "requerido": False,
        "cargado": False,
    },
}
...
_SESSION_STATE: Dict[str, Any] = {
    "session_id": f"CMFG_{int(time.time())}",
    "start_time": time.time(),
    "neuronas_activas": 50,
    "psnrl_sincronizado": False,
    "daemon_ipfs_vinculado": False,
...
def obtener_subredes_disponibles() -> Dict[str, int]:
    ...
    for capa in capas:
        ...
        if dir_capa.exists() and dir_capa.is_dir():
            archivos = [
                f for f in dir_capa.iterdir()
                if f.is_file() and f.suffix in [".py", ".json", ".bin", ""]
            ]
...
def get_session_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "neuronas_activas": _SESSION_STATE["neuronas_activas"],
        "psnrl_sincronizado": _SESSION_STATE["psnrl_sincronizado"],
        "operaciones_completadas": _SESSION_STATE["operaciones_completadas"],
...
def obtener_resumen_psnrl() -> Dict[str, Any]:
    ...
    return {
        ...
        "tamano_kb": round(tam_total / 1024, 2),
...
def guardar_checkpoint_sesion(etiqueta: str = "auto") -> Dict[str, Any]:
    ...
    ckpt = {
        ...
        "etiqueta": etiqueta,
        "sesion": get_session_info(),
        "psnrl": obtener_resumen_psnrl(),
        "topologia": obtener_subredes_disponibles(),
    }
...
def restaurar_ultimo_checkpoint() -> Optional[Dict[str, Any]]:
    ...
    ckpts = sorted(
        PACKAGE_ROOT.glob("_checkpoint_*.json"),
...
def exportar_diagnostico_json(destino_path: Optional[Union[str, Path]] = None) -> Path:
    ...
    reporte = {
        ...
        "version_cmfg": __version__,
        "sesion": get_session_info(),
        "subsistemas": verificar_estado_subsistemas(),
        "topologia": obtener_subredes_disponibles(),
        "psnrl": obtener_resumen_psnrl(),
        "sincronizacion": sincronizar_manifiesto_con_psnrl(),
    }
...
def cerrar_sesion_cmfg(subir_pesos: bool = True) -> Dict[str, Any]:
    ...
    res: Dict[str, Any] = {
        "session_id": _SESSION_STATE["session_id"],
        "hooks": ejecutar_hooks_cierre(),
        "ipfs_resumen": None,
        "temporales_purgados": purgar_temporales(),
    }
    if subir_pesos:
        try:
...
def banner_diagnostico() -> str:
    ...
    lineas = [
        "=" * 68,
        f"  CELEBRO MODEL & FEED GATEWAY (CMFG) v{__version__}",
        "=" * 68,
        f"  Directorio Base : {PACKAGE_ROOT}",
        f"  Directorio PSNRL: {PSNRL_DIR} ({psnrl['total_archivos']} arch, {psnrl['tamano_kb']} KB)",
        f"  Topologia Red   : ENRN={topo.get('ENRN',0)}, RF_EN={topo.get('RF_EN',0)}, "
        f"RF_SL={topo.get('RF_SL',0)}, RNP={topo.get('RNP',0)}, SLRN={topo.get('SLRN',0)}",
        f"  Sesion ID       : {_SESSION_STATE['session_id']}",
        "=" * 68,
    ]
    return "\n".join(lineas)
...
__all__: Final[List[str]] = [
    ...
    "MANIFEST_PATH",
    "inicializar_entorno",
    "registrar_hook_cierre",
    "desregistrar_hook_cierre",
    "ejecutar_hooks_cierre",
    "obtener_subredes_disponibles",
    "verificar_estado_subsistemas",
    "get_session_info",
    "incrementar_contador_operaciones",
    "purgar_temporales",
    "obtener_resumen_psnrl",
    "auto_conectar_ipfs",
    "guardar_checkpoint_sesion",
    "restaurar_ultimo_checkpoint",
    "sincronizar_manifiesto_con_psnrl",

...
class RefactorizadorSesion:
    ...
    def ejecutar(self, mostrar_barra: bool = True) -> Dict[str, Any]:
        """Refactoriza overlay: archivos oversized + clases grandes."""
        ...
        if mostrar_barra:
            _escribir_barra("\n")
        clases = _refactor_clases(self.overlay)
        nc = len(clases.get("extraidas", []))
        return {"exito": fallos == 0 and clases.get("exito", True), "analizados": total,
                "refactorizados": hechos, "clases_extraidas": nc, "fallos": fallos,
                "detalle": detalle,
                "mensaje": f"Version de sesion: {hechos} archivo(s) + {nc} clase(s) a regla 400/450 (modelo local)." if total or nc else "Version de sesion: todo el codigo ya cumple 400/450."}
